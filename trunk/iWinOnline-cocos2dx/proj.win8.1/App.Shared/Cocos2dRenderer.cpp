...
Cocos2dRenderer::Cocos2dRenderer(int width, int height, float dpi, DisplayOrientations orientation, CoreDispatcher^ dispatcher, Panel^ panel, Windows::Foundation::Point scale_factor)
    : m_app(nullptr)
    , m_width(width)
    , m_height(height)
    , m_dpi(dpi)
...
Windows::Foundation::Size Cocos2dRenderer::GetCurrentViewSize()
{
	...
	Windows::Foundation::Size(0, 0);
}

void Cocos2dRenderer::SetScaleFactor(Windows::Foundation::Point scale_factor)
{
	auto director = cocos2d::Director::getInstance();
	auto glview = director->getOpenGLView();
	if (glview)
	{
		GLViewImpl* w_glview = (GLViewImpl*)glview;
		m_scalefactor = scale_factor;
		w_glview->SetScaleFactor(Vec2(scale_factor.X, scale_factor.Y));
	}
}
...
void Cocos2dRenderer::Resume()
{
    ...
    if (!glview) 
    {
        GLViewImpl* glview = GLViewImpl::create("Iwin Online");
		...
		glview->SetScaleFactor(Vec2(m_scalefactor.X,m_scalefactor.Y));
