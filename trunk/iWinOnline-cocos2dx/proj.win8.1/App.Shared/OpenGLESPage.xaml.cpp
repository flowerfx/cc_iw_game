    ...
    mOrientation(DisplayOrientations::Landscape)
{
	...
	view->VisibleBoundsChanged += ref new Windows::Foundation::TypedEventHandler<Windows::UI::ViewManagement::ApplicationView ^, Platform::Object ^>(this, &CocosAppWinRT::OpenGLESPage::OnVisibleBoundsChanged);
...
void OpenGLESPage::OnPageLoaded(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
	...
	grvebview->Visibility = Windows::UI::Xaml::Visibility::Collapsed;
	WPPlatform::WPHelper::hideImageLoadGame = [this]() {
		this->Dispatcher->RunAsync(Windows::UI::Core::CoreDispatcherPriority::Normal,
			ref new Windows::UI::Core::DispatchedHandler([this]()
		{
			grBG->Visibility = Windows::UI::Xaml::Visibility::Collapsed;
		}, CallbackContext::Any));	
	};

	WPPlatform::WPHelper::onShowBtnClear = [this](bool value, float X, float Y) {
		this->Dispatcher->RunAsync(Windows::UI::Core::CoreDispatcherPriority::Normal,
			ref new Windows::UI::Core::DispatchedHandler([this , value, X, Y]()
		{
			if (value)
			{
				btn_clear->Visibility = Windows::UI::Xaml::Visibility::Visible;
				btn_clear->Margin = Thickness(X - 10, 0, 0, Y - 15);
				//btn_clear->
			}
			else
			{
				btn_clear->Visibility = Windows::UI::Xaml::Visibility::Collapsed;
			}
		}, CallbackContext::Any));
	};
...
void OpenGLESPage::CreateRenderSurface()
{
    if (mOpenGLES && mRenderSurface == EGL_NO_SURFACE)
    {
		...
		DisplayInformation^ currentDisplayInformation = DisplayInformation::GetForCurrentView();
		float scale_factor = currentDisplayInformation->RawPixelsPerViewPixel;
#ifdef USE_SCALE
		_scaleFactor = Windows::Foundation::Point(scale_factor, scale_factor);
#endif
		Windows::Foundation::Size size(rect.Width * _scaleFactor.X, rect.Height * _scaleFactor.Y);
...
#endif
#else
	_scaleFactor = Windows::Foundation::Point(1.f, 1.f);
#endif
...
#if ! defined OS_W8
		panelWidth  = rect.Width * this->_scaleFactor.X;
		panelHeight = rect.Height * this->_scaleFactor.Y;
#endif
...
//#ifdef DEBUG
			...
			auto mem = Windows::System::MemoryManager::AppMemoryUsage / (1024 * 1024);
			OutputDebugStringA(("memory" + std::to_string(mem) +  "MB \n").c_str());///1024/1024
#endif
//#endif
...
void CocosAppWinRT::OpenGLESPage::OnVisibleBoundsChanged(Windows::UI::ViewManagement::ApplicationView ^sender, Platform::Object ^args)
{
	DisplayInformation^ currentDisplayInformation = DisplayInformation::GetForCurrentView();
	Windows::Foundation::Size current_size(0, 0);
	if (mRenderer.get())
	{
		current_size = mRenderer->GetCurrentViewSize();
	}

	float height = sender->GetForCurrentView()->VisibleBounds.Height;
	float width = sender->GetForCurrentView()->VisibleBounds.Width;

	float scale_factor = 1.f;
#ifdef USE_SCALE
	scale_factor = currentDisplayInformation->RawPixelsPerViewPixel;
#endif
	float actual_height = scale_factor * height;
	float actual_width = scale_factor * width;

	if (mRenderer.get() && current_size.Width != 0)
	{
		float delta_width = abs(current_size.Width - actual_width);
		float delta_scale = (float)current_size.Width / (actual_width / scale_factor);
		mRenderer->SetScaleFactor(Windows::Foundation::Point(delta_scale, scale_factor));
	}
...
void CocosAppWinRT::OpenGLESPage::btn_clear_Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
	auto textBox = static_cast<Controls::Control^>(cocos2d_canvas->Children->GetAt(0));
	if (textBox)
	{
	//	textBox->
	}
