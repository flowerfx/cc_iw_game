...
Cocos2dRenderer::Cocos2dRenderer(int width, int height, float dpi, DisplayOrientations orientation, CoreDispatcher^ dispatcher, Panel^ panel, float scale_factor)
    : m_app(nullptr)
    , m_width(width)
    , m_height(height)
    , m_dpi(dpi)
...
void Cocos2dRenderer::Resume()
{
    ...
    if (!glview) 
    {
		...
		glview->SetScaleFactor(Vec2(1,1));
