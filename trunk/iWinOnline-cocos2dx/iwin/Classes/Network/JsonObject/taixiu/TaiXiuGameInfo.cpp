...
namespace iwinmesage
{
	...
	rapidjson::Document TaiXiuGameInfo::toJson()
	{
		...
		rapidjson::Value opt(newUserBet.toJson(), locator);
		document.AddMember("newUserBet", opt, locator);
	...
	void TaiXiuGameInfo::toData(rapidjson::Document & document)
	{
		...
		if (document.HasMember("newUserBet"))
		{
			const rapidjson::Value& cnewUserBet = document["newUserBet"];
			rapidjson::StringBuffer buffernewUserBet;
			buffernewUserBet.Clear();
			rapidjson::Writer<rapidjson::StringBuffer> writeUser(buffernewUserBet);
			cnewUserBet.Accept(writeUser);
			TaiXiuUserBet  val;
			val.toData(buffernewUserBet.GetString());
			setNewUserBet(val);
		}
