...
namespace iwinmesage
{
	...
	rapidjson::Document GameList::toJson()
	{
		...
		rapidjson::Value arrgameHasRuby(rapidjson::kArrayType);
		for (size_t i = 0; i < gameHasRuby.size(); i++)
		{
			arrgameHasRuby.PushBack(gameHasRuby.at(i), locator);
		}
		document.AddMember("gameHasRuby", arrgameHasRuby, locator);
	...
	void GameList::toData(rapidjson::Document & document)
	{
		...
		if (document.HasMember("gameHasRuby") && !document["gameHasRuby"].IsNull() && document["gameHasRuby"].IsArray())
		{
			const rapidjson::Value& arrgameHasRuby = document["gameHasRuby"];
			for (rapidjson::SizeType i = 0; i < arrgameHasRuby.Size(); i++)
			{
				gameHasRuby.push_back(arrgameHasRuby[i].GetInt());
			}
		}
	...
	bool GameList::haveRuby(int gameId)
	{
		for (auto g : gameHasRuby)
		{
			if (g == gameId)
			{
				return true;
