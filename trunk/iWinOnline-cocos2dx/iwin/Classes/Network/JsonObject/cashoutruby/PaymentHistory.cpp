...
rapidjson::Value vpin(pin.c_str(), pin.size());
		document.AddMember("pin", vpin,locator);
rapidjson::Value vseri(seri.c_str(), seri.size());
		document.AddMember("seri", vseri,locator);
		
		document.AddMember("chargeId", chargeId,locator);
		document.AddMember("subChargeId", subChargeId,locator);
	...
	void PaymentHistory::toData(rapidjson::Document & document)
	{
		...
		if(document.HasMember("pin"))
		{
			setPin(document["pin"].GetString());
		}
		if(document.HasMember("seri"))
		{
			setSeri(document["seri"].GetString());
		}
		if(document.HasMember("chargeId"))
		{
			setChargeID(document["chargeId"].GetInt());
		}
		if(document.HasMember("subChargeId"))
		{
			setSubChargeID(document["subChargeId"].GetInt());
		}
	}
}
