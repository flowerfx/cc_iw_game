	...
	//try
	{
		...
		case IwinProtocol::RESPOND_QUESTION_FROM_SERVER:
		{
			...
			if (questionSize3001 == 1) { // aMinh nhắn nhủ khi size == 1 thì
										 ...
										 // khác
				globalLogicHandler->onReceiveQuestion(IwinProtocol::RESPOND_QUESTION_FROM_SERVER, questionId[0],
        ...
        case IwinProtocol::CHECK_VALIDATE_EMAIL: {
            std::string ms = msg->readString();
            std::string ms1 = "";
            if (ms == "")
            {
            ...
            globalLogicHandler->onCallBackEmailExist(ms1);
            break;
        }
        case IwinProtocol::NEW_REGISTER: {
            bool isSuccessRegister = msg->readBool();
            std::string ms = msg->readString();
            std::string ms1 = "";
            if(isSuccessRegister) {
                ms1 = msg->readString();
            }
            globalLogicHandler->onRequestRegister(ms1);
                break;
        }
		case 2800:
		...
		case IwinProtocol::SERVER_MESSAGE:
		{
			...
			case 2:
			{
				...
				ScrMgr->OnShowDialog("notice", server_message, DIALOG_TWO_BUTTON, "", [link](const char * btn, const char * name) {
					OPEN_URL(link);
				});
				//globalLogicHandler->onShowDialogResult(true, server_message);
				break;
			}
			case 3:
			{
        ...
        case IwinProtocol::SEND_CAPTURE_IMG:
        {
            ScrMgr->OnHideDialog();
            if (GameController::getLoginType() == AccountTypes::iwin)
            {
                ScrMgr->OnShowOKDialog("your_image_is_uploaded");
            }
            ...
            if (length > 0) {
                ...
                // imgData[i] = msg.readByte();
                // }
                /*
                 * mảng byte này là server scale hình với kích thước bình
                 * thường cho j2me không phải kích thước cho smartphone nên
                 * không cập nhật hình này vào cache, nếu không sẽ bị lỗi
                 * hình bị mờ.
                 *
                 * sau khi nhận được thì gọi lây hình lại.
                 */
                GameController::myInfo->avatar = -1;
                globalLogicHandler->onReceivedMyAvatarCapture(GameController::myInfo->IDDB);
            }
            break;
        }
        break;
		case IwinProtocol::TOAST_MESSAGE:
		{
			std::string text = msg->readString();
			int time = msg->readInt();
			//ScrMgr->OnShowOKDialog(text);
			GetCommonScr->OnShowToast(text);
		}
		break;
		case IwinProtocol::CHAT_TO:
		{
			int fromID = msg->readInt();
			std::string fromName = msg->readString();
			std::string Message = msg->readString();
			break;
		}
		case IwinProtocol::MESSAGE:
		{
			IwinMessage * m = new IwinMessage();
			m->typeN = msg->readInt();
			m->time = msg->readLong();
			m->fromID = msg->readInt();
			m->avatar = GameController::GetFriendAvatar(m->fromID);
			m->fromName = msg->readString();
			m->message = msg->readString();
			if (m->fromID != GameController::myInfo->IDDB && m->toID == -1)
			{
				m->toID = GameController::myInfo->IDDB;
				m->toavatar = GameController::myInfo->avatar;
				m->toName = GameController::myInfo->name;
			}

			m->isRead = false;
			globalLogicHandler->onMessageFrom(m);
			break;
		}
		...
		case IwinProtocol::GET_LIST: 
		{
			...
			else if (type == 2) 
			{
				...
				while (!msg->isEndRead())
				{
					...
					p->setMoney(GameController::currentMoneyType,msg->readLong());
		...
		case IwinProtocol::STRONGEST_LIST: {
			...
			while (!msg->isEndRead())
			{
				...
				info->setMoney(GameController::currentMoneyType, msg->readLong());
		...
		case IwinProtocol::FRIEND:
		{
			...
			switch (id) 
			{
				...
				{
					globalLogicHandler->onShowDialogResult(true, msg->readString());
					GlobalService::requestListWaitingFriend();
					break;
				}
		...
		case IwinProtocol::TRANSFER_INFO:
		{
			int taxWin = msg->readInt();
			int minWin = msg->readInt();
			int maxWIn = msg->readInt();
			int minRuby = msg->readInt();
			int maxRuby = msg->readInt();
			int taxRuby = taxWin;
			globalLogicHandler->onDataTransferDialog(taxWin, taxRuby, minWin, minRuby, maxWIn, maxRuby);
			break;
		}
		case IwinProtocol::INVALID_ACCESS_TOKEN:
		{
			globalLogicHandler->onShowDialogResult(true, "login fail !");
		}
		break;
		case IwinProtocol::CHARGE_MONEY:
		{
			char type = msg->readByte();
			switch (type) {
			case Payment::CHARGE_BY_ID_IAP:
			case Payment::CHARGE_BY_ID_GOOGLE:
				char resultCode = msg->readByte();
				std::string message = msg->readString();
				std::string receip = msg->readString();
				Utils::hideWaitDialog();
				globalLogicHandler->onShowDialogResult(true, message);
				break;
			}
			break;
		}
		case IwinProtocol::GET_OTP_STATUS:
		{
			std::vector<OTPStatus*> list_otp_status;
			while (!msg->isEndRead())
			{
				OTPStatus * otpStatus = new OTPStatus();
				otpStatus->setType(msg->readInt());
				otpStatus->setStatus(msg->readInt());
				otpStatus->setIsDefault(msg->readBool());
				list_otp_status.push_back(otpStatus);
			}
			globalLogicHandler->onReceiveOTPStatus(list_otp_status);
		}
		break;
		case IwinProtocol::CHANGE_DEFAULT_OTP:
		{
			int typeOTP = msg->readInt();
			bool status = msg->readBool();
			std::string desc = msg->readString();
			globalLogicHandler->onReceiveChangeDefaultOTP(typeOTP, status, desc);
		}
		break;
		case IwinProtocol::GET_BEST_ELO:
		{
			int typeN = msg->readByte();
			switch (typeN)
			{
			case 0:
			case 1:
			{
				...
				std::vector<int> arrElo;
				for (int i = 0; i < length; i++) {
					arrID.push_back(msg->readInt());
					arrElo.push_back(msg->readInt());
				}
			}
			break;
			case 2:
			...
			case 3:
			{
				...
				int elo = msg->readInt();
				std::string elon = msg->readString();
				int eloItemId = msg->readInt();
			}
			break;
			}
		}
...
void GlobalMessageHandler::onConnectFail()
{
	ScrMgr->PushEvent([this]() {
		ScrMgr->OnShowDialog("notice", "cannot_connect_server_refresh", DIALOG_TWO_BUTTON, "",
			[](const char * str, const char * name)
		{
			ScrMgr->SwitchToMenu(LOGIN_SCREEN, ScrMgr->GetCurrentMenuUI());
			GlobalService::getServerListFromWeb(
				[](void * data) //done
			{
				auto buffer = (std::vector<char> *)data;
				GetServerMgr()->UpdateServerListFromString(std::string((char*)&(buffer->front()), buffer->size()));
				ScrMgr->updateServerList();
				ScrMgr->OnShowOKDialog("update_successful");
			},
				[]() //fail
			{
				ScrMgr->OnShowErrorDialog("there_are_some_errors_in");
			});
		},
			[](const char * str, const char * name)
		{
