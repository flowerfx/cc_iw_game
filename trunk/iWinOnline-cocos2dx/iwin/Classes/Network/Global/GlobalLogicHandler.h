...
public:
	...
	GlobalLogicHandler();
	~GlobalLogicHandler();
	void onReceiveQuestion(int commandId, int questionid, std::string questionString, int limitTime, std::string info);
    ...
    void onCallBackEmailExist(std::string msg);
    void onRequestRegister(std::string msg);
    ...
    void onReceivedMyAvatarCapture(int iddb);
	...
	*/
	...
	void onDataTransferDialog(int taxWin, int taxRuby, int minWin, int minRuby, int maxWin, int maxRuby);
	void onLoginWithTokenFailed(InvalidLoginToken data);
	void onMustRegisterWithAccessToken(MustRegisterWithToken data);
	void onLoginWithAccessTokenSuccess(LoginWithTokenSuccess data);
	...
	*/
	void onReceiveRequireAuthenOTP(RequireAuthenOTP *  data);

	/*
	*	Nhận được kết quả nếu chứng thực OTP thành công hay thất bại
	*/
	void onReceiveAuthorizeOTP(ResponseAuthorizeOTP* data);

	/*
	*	Receive OTP status
	*/

	void onReceiveOTPStatus(std::vector<OTPStatus *> arr_list);

	/*
		nhan ve status doi trang thai
	*/

	void onReceiveChangeDefaultOTP(int optID, bool status, std::string msg);

	void onUpdateElo(std::vector<int> arrID, std::vector<int> arrElo, int subCommand);

	/*
	* no hu msg 
	*/

	void onReceivedNoHuGameInfo(NoHuGameInfo * data);

	void onReceivedNoHuBet(NoHuBetResult * data);

	void onReceivedNoHuUserHistory(NoHuUserHistoryServer * data);

	void onReceivedNoHuListUser(NoHuListUser * data);

	void onReceivedNoHuUnsupportList(NoHuUnsupportedList * data);
};
#endif