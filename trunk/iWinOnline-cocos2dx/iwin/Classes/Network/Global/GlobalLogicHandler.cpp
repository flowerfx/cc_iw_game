...
#include "../JsonObject/login/InvalidLoginToken.h"
#include "../JsonObject/login/MustRegisterWithToken.h"
#include "../JsonObject/login/LoginWithTokenSuccess.h"
...
void GlobalLogicHandler::onReceiveQuestion(int commandId, int questionid, std::string questionString, int limitTime, std::string info)
{
	ScrMgr->OnHideDialog();
	
	PASSERT2(false, "handle question dialog here !");
	//QuestTionDia

	GlobalService::sendIdQuestionToServer(commandId, questionid);
}
...
void GlobalLogicHandler::onCallBackEmailExist(std::string msg)
{
    CCLOG("email exist from server: %s", msg.c_str());
    ScrMgr->SendDataToScreen(MENU_LAYER::LOGIN_SCREEN, IwinProtocol::CHECK_VALIDATE_EMAIL, (void*)msg.c_str());
}

void GlobalLogicHandler::onRequestRegister(std::string msg)
{
    CCLOG("response register from server: %s", msg.c_str());
    ScrMgr->SendDataToScreen(MENU_LAYER::LOGIN_SCREEN, IwinProtocol::NEW_REGISTER, (void*)msg.c_str());
}
...
void GlobalLogicHandler::onReceiveMessageJson(std::string msg_json, std::string id_msg)
{
	...
	else if (id_msg == IwinCommand::TAIXIU_GET_INFO)
	{
		...
		id_msg == IwinCommand::TAIXIU_GET_UNSUPPORTED_LIST_RUBY)
	{
	...
	else if (id_msg == IwinCommand::TAIXIU_GET_MATCH_HISTORY ||
		id_msg == IwinCommand::TAIXIU_GET_MATCH_HISTORY_RUBY)
	{
		TaiXiuMatch* match = new TaiXiuMatch();
		match->toData(msg_json);

		GetCommonScr->OnReceivedTaiXiuMatch(match);
	}
	else if (id_msg == IwinCommand::TAIXIU_GET_HISTORY ||
		id_msg == IwinCommand::TAIXIU_GET_HISTORY_RUBY)
	{
		TaiXiuMatchList* matchlist = new TaiXiuMatchList();
		matchlist->toData(msg_json);

		GetCommonScr->onReceiveTaiXiuMatchList(matchlist);
	}
	...
	else if (id_msg == IwinCommand::GET_TRANSACTION_HISTORY)
	{
		TransactionHistory* trans_his = new  TransactionHistory();
		trans_his->toData(msg_json);
		ScrMgr->SendDataToScreen(PROFILE_SCREEN, -1, (void*)"GET_TRANSACTION_HISTORY", (void*)trans_his);

	}  // Login
	else if (id_msg == IwinCommand::INVALID_LOGIN_TOKEN)
	{
		InvalidLoginToken data;
		data.toData(msg_json);
		onLoginWithTokenFailed(data);
	}
	else if (id_msg == IwinCommand::MUST_REGISTER_WITH_TOKEN)
	{
		MustRegisterWithToken data ;
		data.toData(msg_json);
		onMustRegisterWithAccessToken(data);
	}
	else if (id_msg ==  IwinCommand::LOGIN_WITH_TOKEN_SUCCESS)
	{
		LoginWithTokenSuccess data;
		data.toData(msg_json);
		onLoginWithAccessTokenSuccess(data);
	}
	else if (id_msg == IwinCommand::REQUIRE_AUTHEN_OTP)
	{
		RequireAuthenOTP *  data = new RequireAuthenOTP();
		data->toData(msg_json);
		onReceiveRequireAuthenOTP(data);
	}
	else if (id_msg == IwinCommand::AUTHORIZE_OTP)
	{
		ResponseAuthorizeOTP * data = new ResponseAuthorizeOTP();
		data->toData(msg_json);
		onReceiveAuthorizeOTP(data);
	}
	else if (id_msg == IwinCommand::NOHU_GET_INFO || id_msg == IwinCommand::NOHU_GET_INFO_RUBY)
	{
		NoHuGameInfo * data = new NoHuGameInfo();
		data->toData(msg_json);
		onReceivedNoHuGameInfo(data);
	}
	else if (id_msg == IwinCommand::NOHU_BET)
	{
		NoHuBetResult * data = new NoHuBetResult();
		data->toData(msg_json);
		onReceivedNoHuBet(data);
	}
	else if (id_msg == IwinCommand::NOHU_GET_USER_HISTORY)
	{
		NoHuUserHistoryServer * data = new NoHuUserHistoryServer();
		data->toData(msg_json);
		onReceivedNoHuUserHistory(data);
	}
	else if (id_msg == IwinCommand::NOHU_GET_LIST_NOHU)
	{
		NoHuListUser * data = new NoHuListUser();
		data->toData(msg_json);
		onReceivedNoHuListUser(data);
	}
	else if (id_msg == IwinCommand::NOHU_GET_UNSUPPORTED_LIST)
	{
		NoHuUnsupportedList * data = new NoHuUnsupportedList();
		data->toData(msg_json);
		onReceivedNoHuUnsupportList(data);
	}
	else
	{
...
void GlobalLogicHandler::onInviteJoinBoard(iwincore::Message* msg)
{
	...
	if (ScrMgr->GetCurrentMenuUI() == LOBBY_SCREEN || ScrMgr->GetCurrentMenuUI() == NEW_LOBBY_SCREEN)
	{
	...
	std::string info)
{
	if (!available) 
	{
		...
		ScrMgr->OnShowOKDialog(text + " " + username);
	}
	else 
	{
...
void GlobalLogicHandler::onReceivedMyAvatarCapture(int iddb)
{
    ScrMgr->OnHideDialog() ;
...
void GlobalLogicHandler::onBuyingAvatarResult(BuyingAvatar * buyingAvatar) 
{
	...
	if (buyingAvatar != nullptr && isValid)
	{
		if (buyingAvatar->getIsSucceed() && curent_money > buyingAvatar->getMoney())
		{
			...
			ScrMgr->MyInfoChanged();
			ScrMgr->OnShowOKDialog(buyingAvatar->getMessage());
		}
...
void GlobalLogicHandler::onVerifyUser(int type_verify, bool is_success, std::string descript)
{
	if (type_verify != 1) 
	{ // email // or otherwise
		ScrMgr->OnShowOKDialog(descript);
	}
	...
	{ // Phone number
		...
		{ 		// Success verify phone numberr
				...
				[](const char * str, const char * btn_name)
			{
				...
				GetCommonScr->OnShowLayer(COMMON_LAYER::CONFIRM_CODE_LAYER);
			});
		}
		else 
		{
...
void GlobalLogicHandler::onReceiveConfirmSMSCode(bool is_success, std::string description)
{
	...
	{
		ScrMgr->OnShowOKDialog(description);
	}
}
...
void GlobalLogicHandler::showDialogDieuHuong(RequestFeature* feature)
{
	...
	if (num_btn == 0)
	{
		...
		if (feature->getButtonOk() != "" && feature->getButtonCancel() != "")
		{
			ScrMgr->OnShowDialog("notice", feature->getMessage(), TYPE_DIALOG::DIALOG_TWO_BUTTON, "", [featureId, gameId](const char* call_back, const char* btn_name) {
				ScrMgr->showScreen(featureId, gameId);
			},
				[featureId, gameId](const char* call_back, const char* btn_name)
			{
		...
		{
			ScrMgr->OnShowDialog("notice", feature->getMessage(), TYPE_DIALOG::DIALOG_ONE_BUTTON, "", [featureId, gameId](const char* call_back, const char* btn_name) {
				ScrMgr->showScreen(featureId, gameId);
			});
		}
...
void GlobalLogicHandler::onTransferMoney(s64 newMoney, std::string info, int moneyType)
{
	...
	if (GetCommonScr->GetLayer(COMMON_LAYER::LUCKY_CIRCLE_LAYER)->IsVisibleLayer())
	{
		ScrMgr->MyInfoChanged();
		return;
	}
	else if (GetCommonScr->GetLayer(COMMON_LAYER::MINI_POKER_LAYER)->IsVisibleLayer() && delta_change_money > 0 )
	{
		GetCommonScr->onSetMoneyBonusNohu(delta_change_money);
		ScrMgr->MyInfoChanged();
		return;
	}
...
void GlobalLogicHandler::onSearchResult(std::vector<Player*> searchList)
{
	...
	{
		ScrMgr->OnShowOKDialog("cannot_find_this_username");
	}
}
...
void GlobalLogicHandler::onAddFriendResult(ubyte addResult)
{
	if (addResult == 0) 
	{
		ScrMgr->OnShowOKDialog("already_sent_reques");
		// thanh cong
	}
	else if (addResult == 2) 
	{
		ScrMgr->OnShowOKDialog("unable_to_add_you_have");
		// qua nhieu ban
	}
	else 
	{
		ScrMgr->OnShowOKDialog("is_already_in_friend");
...
void GlobalLogicHandler::onMessageFrom(IwinMessage * m)
{
	...
	CCLOG("GLOBAL LOGIC HANDLER : onMessageFrom : %s", m->toString().c_str());

	switch (m->typeN)
	{
	case NewFriendScreen::ADDFRIEND:
	{
		REMOVE_ADDFRIEND_MESSAGE(m->fromID);
		PUSH_SINGLE_ADDFRIEND_MESSAGE(m);
		break;
	}
	case NewFriendScreen::CHAT:
	{
		ScrMgr->onReceiveChatMessage(m);
	}
	default:
		break;
	}
...
{
	ScrMgr->OnShowOKDialog(StringUtils::format(LANG_STR("cheers_you_go").GetString(), itemName.c_str()));
...
void GlobalLogicHandler::onDataTransferDialog(int taxWin, int taxRuby, int minWin, int minRuby, int maxWin, int maxRuby)
{
	int * total_arg = new int[6];
	total_arg[0] = taxWin;
	total_arg[1] = taxRuby;
	total_arg[2] = minWin;
	total_arg[3] = minRuby;
	total_arg[4] = maxWin;
	total_arg[5] = maxRuby;
	ScrMgr->SendDataToScreen(PROFILE_SCREEN, IwinProtocol::TRANSFER_INFO, (void*)total_arg);
}

void GlobalLogicHandler::onLoginWithTokenFailed(InvalidLoginToken data)
{
	ScrMgr->SaveDataString(STR_SAVE_GG_STATE, "");
	ScrMgr->SaveDataString(STR_SAVE_FB_STATE, "");
	ScrMgr->SaveTheGame();
	ScrMgr->OnShowOKDialog(data.getReason());
}

void GlobalLogicHandler::onMustRegisterWithAccessToken(MustRegisterWithToken data)
{
	
}

void GlobalLogicHandler::onLoginWithAccessTokenSuccess(LoginWithTokenSuccess data)
{
	if (data.getType() == MainGame::TYPE_FACEBOOK)
	{
		GameController::setLoginType(AccountTypes::facebook);
	}
	else if (data.getType() == MainGame::TYPE_GOOGLE)
	{
		GameController::setLoginType(AccountTypes::google);
	}
    else if (data.getType() == MainGame::TYPE_APPLEID)
    {
...
/*
*	Yêu cầu phải chứng thực OTP
*/
void GlobalLogicHandler::onReceiveRequireAuthenOTP(RequireAuthenOTP * data)
{
	if (ScrMgr->GetCurrentMenuUI() == PROFILE_SCREEN)
	{
		ScrMgr->SendDataToScreen(PROFILE_SCREEN, -1, (void*)"REQUIRE_AUTHEN_OTP", (void*)data);
	}
	else if (ScrMgr->GetCurrentMenuUI() == LOGIN_SCREEN)
	{
        ScrMgr->SendDataToScreen(LOGIN_SCREEN, -1, (void*)"REQUIRE_AUTHEN_OTP", (void*)data);
	}
    else
    {
		delete data;
...
/*
*	Nhận được kết quả nếu chứng thực OTP thành công hay thất bại
*/
void GlobalLogicHandler::onReceiveAuthorizeOTP(ResponseAuthorizeOTP * data) {
	if (data->getResult() == true)
	{
		ScrMgr->OnShowOKDialog(data->getDesc());
		GlobalService::requestOTPStatus();
		if (ScrMgr->GetCurrentMenuUI() == PROFILE_SCREEN)
		{
			ScrMgr->SendDataToScreen(PROFILE_SCREEN, -1, (void*)"AUTHORIZE_OTP", (void*)data);
		}
		else if (ScrMgr->GetCurrentMenuUI() == LOGIN_SCREEN)
		{
			ScrMgr->SendDataToScreen(LOGIN_SCREEN, -1, (void*)"AUTHORIZE_OTP", (void*)data);
		}
		else
		{
			delete data;
	...
	{
		ScrMgr->OnShowErrorDialog(data->getDesc());
		delete data;
	}
}
...
void GlobalLogicHandler::onReceiveOTPStatus(std::vector<OTPStatus *> arr_list) {
	if (ScrMgr->GetCurrentMenuUI() == PROFILE_SCREEN)
	{
		ScrMgr->SendDataToScreen(PROFILE_SCREEN, IwinProtocol::GET_OTP_STATUS, (void*)&arr_list);
	}
	else if (ScrMgr->GetCurrentMenuUI() == LOGIN_SCREEN)
	{
		ScrMgr->SendDataToScreen(LOGIN_SCREEN, IwinProtocol::GET_OTP_STATUS, (void*)&arr_list);
	}
	else
	{
		SAFE_DELETE_VECTOR(arr_list);
	}
}

/*
nhan ve status doi trang thai
*/

void GlobalLogicHandler::onReceiveChangeDefaultOTP(int optID, bool status, std::string msg) {
	if (ScrMgr->GetCurrentMenuUI() == PROFILE_SCREEN)
	{
		ScrMgr->SendDataToScreen(PROFILE_SCREEN, IwinProtocol::CHANGE_DEFAULT_OTP, (void*)&optID, (void*)&status, (void*)(msg.c_str()));
	}
	else if (ScrMgr->GetCurrentMenuUI() == LOGIN_SCREEN)
	{
		ScrMgr->SendDataToScreen(LOGIN_SCREEN, IwinProtocol::CHANGE_DEFAULT_OTP, (void*)&optID, (void*)&status, (void*)(msg.c_str()));
	}
}


void GlobalLogicHandler::onUpdateElo(std::vector<int> arrID, std::vector<int> arrElo, int subCommand)
{
	if ((GameController::getCurrentGameType() == GameType::GAMEID_CARO
		...
		|| GameController::getCurrentGameType() == GameType::GAMEID_CHESS) && subCommand == 1) {
...
void GlobalLogicHandler::onReceivedNoHuGameInfo(NoHuGameInfo * data)
{
	GetCommonScr->onReceivedNoHuGameInfo((void*)data);
}

void GlobalLogicHandler::onReceivedNoHuBet(NoHuBetResult * data)
{
	GetCommonScr->onReceivedNoHuBet((void*)data);
}

void GlobalLogicHandler::onReceivedNoHuUserHistory(NoHuUserHistoryServer * data)
{
	GetCommonScr->onReceivedNoHuUserHistory((void*)data);
}

void GlobalLogicHandler::onReceivedNoHuListUser(NoHuListUser * data)
{
	GetCommonScr->onReceivedNoHuListUser((void*)data);
}

void GlobalLogicHandler::onReceivedNoHuUnsupportList(NoHuUnsupportedList * data)
{
	ScrMgr->setNohuUnsupportedList(data);
}