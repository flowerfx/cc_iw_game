...
void GameMessageHandler::onMessage(iwincore::Message* msg)
{
	...
	case IwinProtocol::BONUS_MONEY:
	{
		...
		_gameLogicHandler->onBonusMoney(roomID, boardID, playerID, money, des, currMoney, moneyType);
	}
	break;
	...
	case IwinProtocol::JOIN_BOARD:
	{
		...
		while (available) {
			...
			if (playerInfo->IDDB == -1) {
				...
				playerInfo->setMoney(GameController::currentMoneyType,msg->readLong());
				...
				if (playerInfo->IDDB == GameController::myInfo->IDDB) {
					...
					{
						GameController::myInfo->setMoney(GameController::currentMoneyType,playerInfo->getMoney());
					}
				}
			}
	...
	case IwinProtocol::ERROR_MESSAGE:
	{
		...
		ScrMgr->OnShowErrorDialog(msg->readString());
	}
	break;
	...
	case IwinProtocol::MONEY_TYPE_INFO:
	{
		...
		if (GameController::listPlayerRubyInGame.size() > 0)
		{
			for (Player* p: GameController::listPlayerRubyInGame)
			{
				delete p;
			}
			GameController::listPlayerRubyInGame.clear();
		}
		for (int i = 0; i < countUser; i++)
		{
			Player* p = new Player();
			p->IDDB = msg->readInt();
			p->setMoney(RUBY_MONEY,msg->readLong());
			GameController::listPlayerRubyInGame.push_back(p);
		}
		if (GameController::currentMoneyType == RUBY_MONEY && GameController::currentGameScreen)
		{
			GameController::currentGameScreen->setRubyMoney(GameController::listPlayerRubyInGame);
		}
