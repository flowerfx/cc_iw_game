...
namespace IwinProtocol
{
	...
	static const s16 IWIN_VERSION = 48;
	...
	*/
	...
	static const s16 LOGIN_WITH_ACCESS_TOKEN = 2301; // login FB
	static const s16 INVALID_ACCESS_TOKEN = 2302; // wrong access token or expired
	static const s16 CREATE_IWIN_ACCOUNT = 2303; // return when first login (have not iwin account yet)
	static const s16 CREATE_IWIN_ACCOUNT_AND_LOGIN = 2304; // send immediately when register iwin account using FB acc
	static const s16 IWIN_ACCOUNT_INFO = 2306; // get location
	...
	*/
    ...
    static const s16 CHECK_VALIDATE_EMAIL = 523;
    static const s16 NEW_REGISTER = 524;
	...
	* */
	...
	/*
		Lay thong tin chuyen win/ruby
	*/
	static const int TRANSFER_INFO = 526;

	/*
		chat in game voi friend
	*/

	/**
	* Active OTP Security
	*/
	static const int ACTIVE_OTP = 1014;

	/**
	* Deactive OTP Security
	*/
	static const int DEACTIVE_OTP = 1015;

	/**
	* đổi phương thức lấy otp khi login
	*/
	static const int CHANGE_OTP_TYPE = 1016;

	/**
	* lấy danh sách OTP của user khi vào màn hình OTP , trả về OTP nào đang
	* active, deactive, dùng cái nào mặc định
	*/
	static const int GET_OTP_STATUS = 1017;

	/**
	* đổi phương thức OTP mặc định.
	*/
	static const int CHANGE_DEFAULT_OTP = 1018;

	/*
	* Yêu cẩu gửi lại OTP (chỉ dùng email hoặc phone)
	*/
	static const int RESEND_OTP = 1019;
};
#endif