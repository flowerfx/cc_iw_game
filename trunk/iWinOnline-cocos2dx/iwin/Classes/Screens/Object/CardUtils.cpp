...
void CardUtils::setSelect(Card* c, bool isSelect)
{
	...
	{
		c->SelectCardAction(isSelect,false);
	}
}
...
std::vector<unsigned char> CardUtils::findAndSelectTwin(int highestCard, std::vector<Card*> cards)
{
		...
		)
	{
		...
		return foundAt;
		//
		for (byte i = 0; i < noCards; i++)
		{
			if (cards[i]->getID() > highestCard)
			{
				byte temp = i;
				// xu ly truong hop sap bai xong xem con quan bai nao nho
				// hon ko
				for (int j = i + 1; j < noCards; j++)
				{
					if (cards[j]->getID() < cards[i]->getID() && cards[j]->getID() > highestCard)
					{
						temp = (byte)j;
					}
				}
				...
				// Tim quân bài cùng số với quân bài hiện tại
				for (byte k = 0; k < noCards; k++)
				{
					if (temp != k && cards[temp]->GetCardValue() == cards[k]->GetCardValue())
					{
						return vector<unsigned char>{(unsigned char)temp, (unsigned char)k};// true;
					}
				}
			}
		}
		*/
		...
		// len
		// mien
		// namnnnnnnnnnnnnnnnnnnnnnnnnnnn
			...
			if (cards[i]->getID() > highestCard)
			{
				...
				for (int j = i + 1; j < noCards; j++)
				{
					if (cards[j]->getID() < cards[i]->getID()
						&& cards[j]->getID() > highestCard)
					{

						// ((Card) cards.get(j)).SetSelect();// = true;
						// return new byte[]{(byte) j};
						temp = (ubyte)j;
					}
				}
				// if (isUseSameColor
				// && highestCard != -1
				// && (player.cards[i] % 4 < 2 && highestCard % 4 > 1 ||
				// player.cards[i] % 4 > 1
				// && highestCard % 4 < 2)) {
				// continue;
				// }
				// Tim quân bài cùng số với quân bài hiện tại
				for (ubyte k = 0; k < noCards; k++)
				{
					if ((temp != k)
						&& cards[temp]->GetCardValue() == cards
							[k]->GetCardValue())
					{
						// if (isUseSameColor
						// && (player.cards[i] % 4 < 2
						// && player.cards[k] % 4 > 1 || player.cards[i] % 4
						// > 1
						// && player.cards[k] % 4 < 2)) {
						// continue;
						// }
						...
						// = true;
						return  vector<ubyte>{ temp,k };// true;
					}
				}
			}
		}
...
std::vector<unsigned char> CardUtils::autoSelecWhenTouchLTMN(TienLenPlayer* player, std::vector<unsigned char> cardSelect, std::vector<unsigned char> curentCards)
{
	...
	else if (cardSelect.size() == 1 && typecard != TYPE_SANH)
	{
		if (curentCards.size() > 0)
		{
			...
			//forAtResult = autoSelect(curentCards, player);
		...
		if (forAtResult.size() > 0)
		{
			...
			for (size_t i = 0; i < cardSelect.size(); i++)
			{
				...
				for (size_t j = 0; j < forAtResult.size(); j++)
				{
					if (cardSelect[i] == listCards[forAtResult[j]]->getID())
					{
						//lsResult.push_back(listCards[forAtResult[j]].id);
						isExits = true;
					}
				}
				if (!isExits)
...
std::vector<unsigned char> CardUtils::autoWhenMultiSelectTLMN(TienLenPlayer* player, std::vector<unsigned char> cardSelect, std::vector<unsigned char> curentCards)
{
	std::vector<unsigned char> tmpReturn;
	if (cardSelect.size() < 2)
	{
		return tmpReturn;
	}
	vector<Card*> listCards = player->cards;
	int typecard = getType(curentCards);
	sort(cardSelect);
	int hightCard = cardSelect[0];
	if (curentCards.size() > 0)
	{
		hightCard = curentCards[0];
	}

	vector<unsigned char> forAtResult;
	if (curentCards.size() <= 0)//bai chon binh thuong
	{
		forAtResult = findDoubleSequence(hightCard, listCards, 4);

		bool isCheckEx = checkArrayItemContain(getListIdToListIndex(listCards, forAtResult), cardSelect);
		if (forAtResult.size() <= 0 || !isCheckEx)
		{
			forAtResult = findAndSelectQuart(hightCard, listCards);
			isCheckEx = checkArrayItemContain(getListIdToListIndex(listCards, forAtResult), cardSelect);
			if (forAtResult.size() <= 0|| !isCheckEx)
			{
				forAtResult = findDoubleSequence(hightCard, listCards, 3);
				isCheckEx = checkArrayItemContain(getListIdToListIndex(listCards, forAtResult), cardSelect);
				if ((forAtResult.size() <= 0 || !isCheckEx) && cardSelect.size() >= 2)
				{
					int numSanh ;
					int typeSelect = getType(cardSelect);
					if (cardSelect.size() == 2 && typeSelect != TYPE_2)
					{
						numSanh = listCards.size();
					}
					else
					{
						numSanh = cardSelect.size();
						forAtResult = findAndSelectTriple(hightCard, listCards);
						isCheckEx = checkArrayItemContain(getListIdToListIndex(listCards, forAtResult), cardSelect);
					}
					while ((forAtResult.size() <= 0 ) && numSanh >= 3)
					{
						forAtResult = findAndSelectSeries(curentCards, hightCard,listCards, numSanh);
						numSanh--;
					}
					isCheckEx = checkArrayItemContain(getListIdToListIndex(listCards, forAtResult), cardSelect);
					if (!isCheckEx)
	...
	if (forAtResult.size() > 0)
	{
		tmpReturn = getListIdToListIndex(listCards, forAtResult);
		if (!checkArrayItemContain(tmpReturn, cardSelect))
		{
			tmpReturn.clear();
		}
	}
...
bool CardUtils::checkArrayItemContain(std::vector<unsigned char> arr1, std::vector<unsigned char> arr2)
{
...
std::vector<unsigned char> CardUtils::getListIdToListIndex(std::vector<Card*> list_cards, std::vector<unsigned char> lsindex)
{
	std::vector<unsigned char> tmpReturn;
	for (int i = 0; i < lsindex.size(); i++)
	{
		tmpReturn.push_back(list_cards[lsindex[i]]->getID());
	}
	return tmpReturn;
}
...
std::vector<unsigned char> CardUtils::autoSelect(std::vector<unsigned char> currentCards, TienLenPlayer* player)
{
	...
	if (foundAt.size() <= 0)
	{
		std::string current_card_fire = "";
		for (auto c : currentCards)
		{
			current_card_fire += ("_" + std::to_string((int)c));
		}
		CCLOG("TLMN CardUtils::autoSelect current_card_fire: %s ", current_card_fire.c_str());
		std::string current_select_card = "";
		for (auto c : currentSelectedCards)
		{
			current_select_card += ("_" + std::to_string((int)c));
		}
		CCLOG("TLMN CardUtils::autoSelect currentSelectedCards: %s , currentHighest: %d ", current_select_card.c_str() , (int)currentHighest);
		auto remain_card = player->getRemainCard();
		std::string str_remain_card = "";
		for (auto c : remain_card)
		{
			str_remain_card += ("_" + std::to_string((int)c));
		}
		CCLOG("TLMN CardUtils::autoSelect remain_card: %s ", str_remain_card.c_str());
