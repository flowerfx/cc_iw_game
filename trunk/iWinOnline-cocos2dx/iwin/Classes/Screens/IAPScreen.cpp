...
const char* s_default_img_name = "otherscreen/topup-screen/loaddefaultnho.png";
const char* s_default_lon_img_name = "otherscreen/topup-screen/loaddefaultlon.png";

#define USE_WP_IAP_STORE 0
...
int IAPScreen::Init()
{
	...
	_spr_captcha->setFixedSize(cocos2d::Size(captcha_bounding->GetSize().x, captcha_bounding->GetSize().y));
...
int	IAPScreen::InitComponent()
{
	...
	p_textfield_bin = GetWidgetChildByName("iap_ui.layout_card.input_cards.panel_bin_input.text_input_bin");
	p_textfield_serial = GetWidgetChildByName("iap_ui.layout_card.input_cards.panel_serial_input.text_input_serial");

	p_layout_btn_award = GetWidgetChildByName("iap_ui.top_panel.btn_change_award");
...
int IAPScreen::Update(float dt)
{
	...
	if (_check_downloading && GetSpriteIapMgr()->isDownloading())
	{
		Utils::hideWaitDialog();
		_check_downloading = false;
	}
...
void IAPScreen::OnProcessWidgetChild(const RKString & name, UI_TYPE type_widget, WidgetEntity * _widget)
{
	if (type_widget == UI_TYPE::UI_BUTTON)
	{
		...
		else if (name == "buy_btn")
		{
			...
			Utils::startWaitDialog(LangMgr->GetString("sending_top_up_message").GetString());
		}
		else if (name == "btn_item_sms")
		{
			//GetIapMgr()->onPurchaseSuccess("android", "\{\"orderId\":\"GPA.1397 - 3633 - 8703 - 67496\",\"packageName\":\"iwin.vn.full\",\"productId\":\"iwin.vn.full.1\",\"purchaseTime\":1477539248818,\"purchaseState\":0,\"developerPayload\":\"198c0637bb58f6bd59b83c0998c239eb\",\"purchaseToken\":\"eodkipiacpfobohngjjbghoa.AO - J1Oyk - izRn7uOzkCMtFk3xM3CRFU_MMZ3oSkTHlYL - RH_7Sy1NnKwF7NMGt6yDdC - 6IbA5u5zatsdtDuAPPjjtME4 - afBvZStS8ITUMyQEdFu3txgC1s\"\}", "");
			Payment* p = (Payment*)_widget->GetResource()->getUserData();
			ScrMgr->OnShowDialog("notice", p->data->getConfirmSMS(), TYPE_DIALOG::DIALOG_TWO_BUTTON, "confirm_sms", [&](const char * call_back, const char* btn_name)
			{
			});
		}
		...
		{
			Payment* p = (Payment*)_widget->GetResource()->getUserData();
			GetIapMgr()->purchase(p->getCode(), p->getPayload());
			ScrMgr->OnShowWaitingDialog();
			//GlobalService::sendChargeMoneyForInApp("iwin.vn.full.1", "", "win");
			//GetIapMgr()->onPurchaseSuccess("android", "\{\"orderId\":\"GPA.1397 - 3633 - 8703 - 67496\",\"packageName\":\"iwin.vn.full\",\"productId\":\"iwin.vn.full.1\",\"purchaseTime\":1477539248818,\"purchaseState\":0,\"developerPayload\":\"198c0637bb58f6bd59b83c0998c239eb\",\"purchaseToken\":\"eodkipiacpfobohngjjbghoa.AO - J1Oyk - izRn7uOzkCMtFk3xM3CRFU_MMZ3oSkTHlYL - RH_7Sy1NnKwF7NMGt6yDdC - 6IbA5u5zatsdtDuAPPjjtME4 - afBvZStS8ITUMyQEdFu3txgC1s\"\}", "");
		}
		...
		{ 
			if (!_payment_win)
			{
				...
				Utils::startWaitDialog("Load IAP");
			}
			else
			{
		...
		{
			ScrMgr->SwitchToMenu(CHANGE_AWARD_SCREEN, IAP_SCREEN);
		}
		else if (name == "btn_refresh_captcha")
		{
			GlobalService::getCaptchaRefresh(_cur_captcha->getId());
		}
		else if (name == "btn_paste_bin")
		{
			Platform::getClipboardText([this](std::string text) {
				static_cast<TextFieldWidget*>(p_textfield_bin)->SetText(text.c_str());
			});

		}
		else if (name == "btn_paste_serial")
		{
			Platform::getClipboardText([this](std::string text) {
				static_cast<TextFieldWidget*>(p_textfield_serial)->SetText(text.c_str());
			});
			//static_cast<TextFieldWidget*>(p_textfield_serial)->SetText(Platform::getClipboardText());
		}
		else if (name == "btn_verify")
		{
			ScrMgr->SwitchToMenu(PROFILE_SCREEN, IAP_SCREEN);
		}
	}
...
void IAPScreen::updateItemScrollViewValueCard(IwinScrollView* scrollview, WidgetEntity* entity, int idx)
{
	...
	static_cast<TextWidget*>(entity->GetWidgetChildByName(".lbl_value_card"))->SetText(StringUtils::format("%s VND", Utils::formatNumber_dot(money).c_str()));
	static_cast<TextWidget*>(entity->GetWidgetChildByName(".lbl_win"))->SetText(StringUtils::format("%s", Utils::formatNumber_dot(win).c_str()));
}

void IAPScreen::initItemScrollView(IwinScrollView* scrollview, WidgetEntity* entity, int idx)
{
	auto btn = entity->GetWidgetChildByName(".btn_item");
	p_panel_iap_method->GetParentLayer()->AddEventType(xml::TYPE_UI_WIDGET::WIDGET_BUTTON, btn);

	if (scrollview == _scroll_view_sms)
	{
		btn->SetName("btn_item_sms");
		entity->GetWidgetChildByName(".icon_card")->SetVisible(false);
	}
	else if (scrollview == _scroll_view_iap)
	{
		btn->SetName("btn_item_iap");
		entity->GetWidgetChildByName(".icon_card")->SetVisible(false);

	}
	else if (scrollview == _scroll_view_banking)
	{
		btn->SetName("btn_item_banking");
...
void IAPScreen::updateItemScrollView(IwinScrollView* scrollview, WidgetEntity* entity, int idx)
{
	...
	if (scrollview == _scroll_view_sms)
	{
		...
		entity->GetWidgetChildByName(".btn_item_sms")->Visible(true);
		if (win != 0)
		{
			static_cast<TextWidget*>(entity->GetWidgetChildByName(".top_lbl"))->SetText(StringUtils::format("%s", Utils::formatNumber_dot(win).c_str()));
		}
		else
		{
		...
		static_cast<TextWidget*>(entity->GetWidgetChildByName(".bot_lbl"))->SetText(StringUtils::format("%s VND", Utils::formatNumber_dot(money).c_str()));
	}
	else if (scrollview == _scroll_view_banking)
	{
	...
	else if (scrollview == _scroll_view_iap)
	{
		...
		static_cast<TextWidget*>(entity->GetWidgetChildByName(".top_lbl"))->SetText(StringUtils::format("%s", Utils::formatNumber_dot(win).c_str()) , true);
		static_cast<TextWidget*>(entity->GetWidgetChildByName(".bot_lbl"))->SetText(StringUtils::format("%.2f $", money), true);
	}
}
...
void IAPScreen::initItemScrollViewTelco(FocusScrollView* scrollview, WidgetEntity* entity, int idx)
{
	...
	static_cast<Button*>(entity->GetWidgetChildByName(".btn_telco")->GetResource())->setTouchEnabled(true);
}
...
void IAPScreen::updateItemScrollViewTelco(FocusScrollView* scrollview, WidgetEntity* entity, int idx)
{
	...
	p_panel_iap_method->GetParentLayer()->AddEventType(xml::TYPE_UI_WIDGET::WIDGET_BUTTON, entity->GetWidgetChildAtIdx(2));
...
{
	int _level = level;
	//CCLOG("AAAAAAAAAAAAAAAAAAAA %s", payment->getCharge_type().c_str());
	if (payment->getType() == "list")
	{
		...
		else if (payment->getCharge_type() == "input_2_field")
		{
			...
			payment->getCharge_type() == "apple_macos_app_store" 
			|| payment->getCharge_type() == "google_app_store"
			|| payment->getCharge_type() == "apple_app_store"
#if USE_WP_IAP_STORE
			|| payment->getCharge_type() == "inapp_wp"
#endif
			)
		{

			//CCLOG("TTTTTTTTTTTTTTTTTTTTTTT data inapp list");
			if (level == 1)
			{
				_payment_method_data.push_back(payment);
			}
	...
	else if (payment->getType() == "action")
	{
		...
		{
			CCLOG("TTTTTTTTTTTTTTTTTTTTTTT data inapp action");
...
void IAPScreen::showPayment(int idx)
{
	...
	std::string charge_type = _payment_method_data.at(idx)->getCharge_type();
	if ( charge_type == "sms")
	{
		showSms();
	}
	else if (charge_type == "input_2_field")
	{
		showCards();
	}
	else if (charge_type == "web")
	{
		showBanking();
	}
	else if (charge_type == "apple_macos_app_store"
		||charge_type == "google_app_store" 
		|| charge_type == "apple_app_store"
#if USE_WP_IAP_STORE
		|| charge_type == "inapp_wp"
#endif
		)
	{ 
		showIap();
...
{
	if (_telco_data_sms.size() <= 0 || telco_idx < 0 || telco_idx >= _telco_data_sms.size())
		return;
...
{
	if (_telco_data_card.size() <= 0 || telco_idx < 0 || telco_idx >= _telco_data_card.size())
		return;
	_scroll_view_value_card->reload(_telco_data_card.at(telco_idx)->data->getItems().size());
}

int IAPScreen::SetIAPStyle(int style)
{
	_iap_style = style;
...
void IAPScreen::OnBeginFadeIn()
{
	...
	if (_iap_style < 0)
	{
		GlobalService::requestPaymentInfo(GameController::getMoneyType().c_str());
	}
	else if (_iap_style == 0)
	{
		GlobalService::requestPaymentInfo("win");
	}
	else
	...
	if (_iap_style < 0)
	{
		...
		{
			p_layout_btn_award->Visible(false);
			GetWidgetChildByName("iap_ui.top_panel.title_win")->Visible(true);
			GetWidgetChildByName("iap_ui.top_panel.title_ruby")->Visible(false);
		}
		else if (GameController::currentMoneyType == RUBY_MONEY)
		{
			p_layout_btn_award->Visible(true);
			p_layout_btn_award->GetWidgetChildByName(".line_glow_1")->SetActionCommandWithName("MOVE");
			p_layout_btn_award->GetWidgetChildByName(".line_glow_2")->SetActionCommandWithName("MOVE");
			p_layout_btn_award->GetWidgetChildByName(".star_glow")->SetActionCommandWithName("ROTATE");

			GetWidgetChildByName("iap_ui.top_panel.title_win")->Visible(false);
			GetWidgetChildByName("iap_ui.top_panel.title_ruby")->Visible(true);
		}
	}
	else if (_iap_style == 0)
	{
...
void IAPScreen::onReceiveCaptchaResult(CaptchaResult* result)
{
	...
	{
		int focus = _scroll_view_telco_card->getItemFocus();
		int charge_by_id = _telco_data_card[focus]->data->getCharge_by_id();
		int charge_by_subid = _telco_data_card[focus]->data->getCharge_by_subid();
		std::string bin = static_cast<TextFieldWidget*>(p_textfield_bin)->GetText().GetString();
		std::string serial = static_cast<TextFieldWidget*>(p_textfield_serial)->GetText().GetString();

		if (bin != "" && serial != "")
		{
			GlobalService::sendChargeMoney(charge_by_id, charge_by_subid, serial, bin, GameController::getMoneyType());

		}
		else
		{
			Utils::hideWaitDialog();
			Utils::showToast(LangMgr->GetString("fill_full_info").GetString());
		}
	}
	else
	{
		ScrMgr->OnShowErrorDialog(result->getDesc());
	}
}
...
void IAPScreen::onReceivePayment(iwinmesage::Payment* payment)
{
	...
	CC_SAFE_DELETE(_payment_win);
	CC_SAFE_DELETE(_payment_ruby);

	if (payment->getError())
	{
		//showWinBtn(true);
		GetWidgetChildByName("iap_ui.layout_sms")->Visible(false);
		GetWidgetChildByName("iap_ui.layout_card")->Visible(false);
		GetWidgetChildByName("iap_ui.layout_banking")->Visible(false);
		GetWidgetChildByName("iap_ui.layout_iap")->Visible(false);
		GetWidgetChildByName("iap_ui.layout_iap_method")->Visible(false);
		_payment_err = payment->getError();
		showVerify(_payment_err);
		CC_SAFE_DELETE(payment);
		return;
	}

	GetWidgetChildByName("iap_ui.layout_verify")->SetVisible(false);
	GetWidgetChildByName("iap_ui.layout_iap_method")->Visible(true);
...
void IAPScreen::reloadPayment()
{
	...
	_check_downloading = true;
}

void IAPScreen::showWinBtn(bool visible)
{
	p_group_button_win->GetWidgetChildByName(".icon_select")->SetVisible(visible);
	p_group_button_ruby->GetWidgetChildByName(".icon_select")->SetVisible(!visible);
}

void IAPScreen::showVerify(int error)
{
	GetWidgetChildByName("iap_ui.layout_verify")->SetVisible(true);
	std::string content_id;
	std::string button_id;
	if (error == 1)
	{
		content_id = "verify_content1";
		button_id = "verify_now";
	}
	else
	{
		button_id = "login";
		content_id = "verify_content2";
	}
	static_cast<TextWidget*>(GetWidgetChildByName("iap_ui.layout_verify.verify_content"))->SetText(LangMgr->GetString(content_id).GetString());
	static_cast<TextWidget*>(GetWidgetChildByName("iap_ui.layout_verify.btn_verify.text"))->SetText(LangMgr->GetString(button_id).GetString());
}

void IAPScreen::SetActionAfterShow(int action)
{
	SetIAPStyle(action);
	this->PushEvent([](BaseScreen * scr) {
		IAPScreen * iap_scr = (IAPScreen*)scr;
		iap_scr->OnBeginFadeIn();
	}, TIME_ACTION);
}