...
{
	SetGameID(GameType::GAMEID_PHOM);
	_card_get_now = -1;
	_cur_fire_cardID = -1;
	SetMaxPlayer(4);
...
bool PhomScreen::InitGame(int game_id)
{
	...
	p_panel_game_card = p_panel_custom->GetWidgetChildByName(".game_card_layer");
	...
	for (int i = 0; i < p_max_player; i++)
	{
		...
		_cardPhomBox[i]->SetWidget(p_panel_custom->GetWidgetChildByName(".result_" + std::to_string(i)));
	}
	p_star_phom = p_panel_custom->GetWidgetChildByName(".star_phom");
	...
	{
		it->SetVisible(false);
        static_cast<Button*>(it->GetResource())->setSwallowTouches(false);
	}
...
void PhomScreen::LoadXMLGameScreenCustom(TiXmlDocument * p_objectXML)
{
	...
	ParseIntoCustom(p_list_sample_dec.at("result_0"), p_panel_custom, "panel_custom_menu");
	ParseIntoCustom(p_list_sample_dec.at("result_1"), p_panel_custom, "panel_custom_menu");
	ParseIntoCustom(p_list_sample_dec.at("result_2"), p_panel_custom, "panel_custom_menu");
	ParseIntoCustom(p_list_sample_dec.at("result_3"), p_panel_custom, "panel_custom_menu");
	ParseIntoCustom(p_list_sample_dec.at("star_phom"), p_panel_custom, "panel_custom_menu");
...
void PhomScreen::OnProcessWidgetChild(const RKString & name, UI_TYPE type_widget, WidgetEntity * _widget)
{
...
void PhomScreen::setSeat(std::vector<Player*> l_players)
{
	//for (int i = 0; i < l_players.size(); i++)
	//{
	//	Player* p = l_players[i];
	//	if (p && p->IDDB == GameController::myInfo->IDDB)
	//	{
	//		//myInfo = players[i];
	//		seats[0] = (byte)i;
	//		break;
	...
	//	}
	//	seats[i] = (byte)i1++;
	//	if (!l_players[seats[i]] || l_players[seats[i]]->IDDB < 0)
	//	{
	//		seats[i] = -1;
	//	}
	//}
...
void PhomScreen::onSomeOneJoinBoard(int seat, Player * p)
{
...
void PhomScreen::onSomeOneLeaveBoard(int leave, int newOwner)
{
	GameScreen::onSomeOneLeaveBoard(leave, newOwner);

	if (leave == _down_first_player)
	{
		p_star_phom->Visible(false);
	}
}
...
void PhomScreen::initFake()
{
	...
	//SetInteractiveWidget(p_btn_fire_card,false);
	setInfo(0, 3);
	setInfo(1, 4);
	setInfo(2, 4);
	setInfo(3, 3);
...
{
	if (cardId >= 0 && cardId < _card_pools.size())
	{
...
void PhomScreen::onFire(int currentPlayer, int nextPlayer, int fireCardID, int numCards)
{
    ...
    CCLOG("onFire --> card %d from curPlayer %d [seat %d] to nextPlayer %d [seat %d]", fireCardID, currentPlayer, GetSeatFromPlayerID(currentPlayer), nextPlayer, GetSeatFromPlayerID(nextPlayer));
	//removeCardFromBox(fireSeat, fireCardID, CardPhomBox.FLOOR_FREE, false);
	_cardPhomBox[fireSeat]->fireCard(fireCardID);
	
    //disable high light on other my free cards
    for (size_t i = 0; i < _myFreeCards.size(); i++) {
        getCard(_myFreeCards.at(i))->showHighlight(false);
    }
    //disable hight light on other player's fired cards
    for (int i = 0; i < p_max_player; i++)
    {
        _cardPhomBox[i]->hideHightLightFiredCards();
    }
...
void PhomScreen::setDownFirstPlayer(int userID)
{
	...
	PlayerGroup * pg = getPlayerGroup(_down_first_player);
	if (pg)
	{
		float delta_scale = GetGameSize().width / this->GetDesignSize().x;
		float delta_pos = 10 * delta_scale;
		if (_down_first_player == _myInfo->GetIDDB())
		{
			delta_pos = 0;
		}
		p_star_phom->Visible(true);
		auto pos = pg->getWidget()->GetPosition_TopLeft();
		p_star_phom->SetPosition(Vec2(pos.x  + delta_pos, pos.y + p_panel_bottom->GetSize().y - delta_pos));
		p_star_phom->GetResource()->stopAllActions();
		p_star_phom->GetResource()->runAction(RepeatForever::create(Sequence::create(DelayTime::create(3.f), FadeOut::create(0.5), FadeIn::create(0.5f) , nullptr)));
	}
	else
	{
		p_star_phom->Visible(false);
...
void PhomScreen::showGameButton(bool changePhom, bool getCard, bool eat, bool fire, bool downPhom, bool sendCard)
{
	...
	SetInteractiveWidget(p_btn_fire_card,fire);
    ...
    if(p_btn_fire_card->IsVisible())
    {
        static_cast<Button*>(p_btn_fire_card->GetResource())->setBright(getCard || sendCard ? false : true);
        static_cast<Button*>(p_btn_fire_card->GetResource())->setTouchEnabled(getCard || sendCard ? false : true);
    }
}
...
void PhomScreen::layoutListBtn()
{
    ...
    {
        num_btn_visible += _list_btns[i]->IsVisible() ? 1 : 0;
    }
	...
	for (size_t i = 0; i < _list_btns.size(); i++)
	{
        static_cast<Button*>(_list_btns[i]->GetResource())->setBright(true);
        static_cast<Button*>(_list_btns[i]->GetResource())->setTouchEnabled(true);
		if (_list_btns[i]->IsVisible()) {
			_list_btns[i]->SetPosition(Vec2(game_size.x - off_btn - (btn_size.x + off_btn) * idx, posY));
//            static_cast<Button*>(_list_btns[i]->GetResource())->setBright(num_btn_visible == 1 || idx > 0);
//            static_cast<Button*>(_list_btns[i]->GetResource())->setTouchEnabled(num_btn_visible == 1 || idx > 0);
            idx++;
		}
	}
}
...
void PhomScreen::addMyCardsToMyBox(bool addSendCard) {
    ...
    for (size_t i = 0; i < _myFiredCards.size(); i++) {
        CCLOG("add card in seat [0] to my box fire: %d", _myFiredCards.at(i));
...
/*	Log.log("addMyCardsToMyBox",
		".........................................")*/;
    PLAY_SOUND("SFX_GAMEBAI_CHIABAI");
...
void PhomScreen::sortFreeCard() {
	...
	for (size_t i = 0; i < _myPhoms.size(); i++) {
		for (size_t j = 0; j < _myPhoms.at(i)._cards.size(); j++) 
		{
//            if (_myPhoms.at(i)._cards.at(j) < 0 || _myPhoms.at(i)._cards.at(j) >=_card_pools.size())
//            {
//                int i = 0;
//            }
...
void PhomScreen::resetGame()
{
	...
	p_btn_create_phom->Visible(false);
	p_btn_down_phom->SetDisable(false);
	p_btn_eat_card->SetDisable(false);
	p_btn_fire_card->SetDisable(false);
	p_btn_get_card->SetDisable(false);
	p_btn_send_card->SetDisable(false);
	p_btn_change_phom->SetDisable(false);
	p_btn_create_phom->SetDisable(false);
	//p_btn_un
	...
	if (_cur_move_player == _myInfo->getPlayer()->IDDB && lastUserNumFiredCards < 4) {
		...
		*/
		...
		if (!_isDownMyPhom) {
            ...
            /*
             * xử lý remove bài đã đánh trong myDealCards ra
             */
            VecByte allDealCards;
            for (size_t i = 0; i < _myDealCards.size(); i++) {
                if (!K_VEC_CONTAIN(_myFiredCards, _myDealCards.at(i)))
                {
                    allDealCards.push_back(_myDealCards.at(i));
                }
            }
            
			canEat1 = PhomUtils::canEat(_cur_fire_cardID, _myEatCards,
				/*_myDealCards*/allDealCards, needCards1, newPhoms1, newFreeCards1);
		...
		if (canEat) {
			for (size_t i = 0; i < needCards.size(); i++) {
                ...
                //show arrow cho la bai co the an dc va touch action vao la bai --> action eat
                Card* firedCard = getCard(_cur_fire_cardID);
                if(firedCard)
                {
                    showArrowAt(firedCard);
                }
			}
	...
	if (_cur_move_player == _myInfo->GetIDDB())
	{
		...
		if (selectedCards.size() == 1) {
			if(K_VEC_CONTAIN(_myEatCards, selectedCards.at(0))){
				...
				if (!_isDownMyPhom) {
					...
					if (PhomUtils::canFire(selectedCards.at(0), _myDealCards,
						...
						PhomService::fire(_roomID, _boardID,
							selectedCards.at(0));
						SetInteractiveWidget(p_btn_fire_card,false);
						SetInteractiveWidget(p_btn_change_phom,false);
						SetInteractiveWidget(p_btn_create_phom,false);
					}
					else {
                        PLAY_SOUND("SFX_POONG");
						//GameController.playSound(iWinSound.POONG);
				...
				// theo rule moi do van con phom chua ha ben duoi nhung
				// van duoc danh
				else {
            ...
            //auto down all card selected 
            _cardPhomBox[0]->unselectAllCardSelected();
		}
		else
        {
            PLAY_SOUND("SFX_POONG");
...
{
    hideArrow();
    Card* card = getCard(_cur_fire_cardID);
    if(card)
    {
	...
	SetInteractiveWidget(p_btn_eat_card,false);
	SetInteractiveWidget(p_btn_get_card,false);
	SetInteractiveWidget(p_btn_change_phom,false);
	SetInteractiveWidget(p_btn_create_phom,false);
...
void PhomScreen::onGetCard(int userID, byte cardID) {
    ...
    CCLOG("get card for user %d [seat %d] with card %d", userID, GetSeatFromPlayerID(userID), cardID);
	...
	if (userID == _myInfo->GetIDDB()) 
	{
		...
		SetInteractiveWidget(p_btn_eat_card,true);
		SetInteractiveWidget(p_btn_get_card,true);
		//cardPhomBoxs[0].setTouchable(Touchable.enabled);
		SetInteractiveWidget(p_btn_change_phom,true);
		SetInteractiveWidget(p_btn_create_phom,true);
        ...
        _cardPhomBox[seat]->addFreeCard(cardID);
        _cardPhomBox[seat]->layoutBox();
        PLAY_SOUND("SFX_GAMEBAI_CHIABAI");
	}
...
void PhomScreen::onEatCard(int downFirst) {
	_on_reconnect = false;
	if (_cur_fire_cardID >= 0 && _cur_fire_cardID < _card_pools.size())
	{
	...
	if (_last_move_player == _myInfo->GetIDDB()) {
		// mình bị ăn
		PLAY_SOUND("SFX_GAMEBAI_BIANBAI");
		//		GameController.playSound(iWinSound.BI_AN_GA);
	}
	PLAY_SOUND("SFX_GAMEBAI_ANBAI");
    ...
    CCLOG("onEatCard --> lastMove %d [seat %d], curMove %d [seat %d], downFirst %d [seat %d]", _last_move_player, seatLastMove, _cur_move_player, seatCurrentMove, _down_first_player, seatDownFirst);
    CCLOG("onEatCard --> curFire %d", _cur_fire_cardID);
    
	if (seatLastMove >= 0 && seatLastMove < 4)
	{
	...
	if (_cur_move_player != _myInfo->GetIDDB()) 
	{
        //addCardToBox(seatCurrentMove, _cur_fire_cardID, FLOOR_PHOM, true, false);
        ...
        //highlight la bai an ga
        getCard(_cur_fire_cardID)->GetResource()->setColor(Color3B(251, 255, 171));
	}
...
//            removeCardFromBox(seatDownFirst, lastFiredCard_DownFirst, CardPhomBox.FLOOR_FIRED, false);
//            addCardToBox(seatLastMove, lastFiredCard_DownFirst, CardPhomBox.FLOOR_FIRED, false, false);
            CCLOG("onEatCard --> xoay rac %d from seat %d to seat %d", lastFiredCard_DownFirst, seatDownFirst, seatLastMove);
            ...
            if (seatDownFirst == 0)
            {
                //la minh thi xoa luon trong _myFiredCards
                K_VEC_REMOVE(_myFiredCards, lastFiredCard_DownFirst);
            }
            
            if(seatLastMove == 0)
            {
                //la minh thi add them trong fireCard
                _myFiredCards.push_back(lastFiredCard_DownFirst);
            }
            
            _cardPhomBox[seatDownFirst]->layoutBox();
            _cardPhomBox[seatLastMove]->layoutBox();
            
            //here check wrong logic
            bool b0 = _cardPhomBox[seatDownFirst]->hasFireCard((int)lastFiredCard_DownFirst);
            bool b1 = _cardPhomBox[seatLastMove]->hasFireCard((int)lastFiredCard_DownFirst);
            if(b0 && b1)
            {
                int t = 0;
            }
...
//	}
    ...
    for (Card *c : free_cards)
    {
        if (NODE(c)->getTag() == 1)
        {
            c->SelectCardAction(!c->isSelected() , false, true);
        }
        //c->showHighlight(false);
        NODE(c)->setTag(0);
    }
    
    //handle touch on fired card can be ate by myself
//    if(_last_action == ACTION_EAT)
//    {
//        int seat = GetSeatFromPlayerID(_last_move_player);
//        if(seat != -1)
//        {
//            Card* firedCard = getCard(_cardPhomBox[seat]->getLastFireCardID());
//            if(NODE(firedCard) && NODE(firedCard)->getTag() == 1)
//            {
//                //action like button eat card
//                NODE(firedCard)->setTag(0);
//                actionEat();
//            }
//        }
//    }
...
void PhomScreen::ActionTouchBeginObject(Utility::wObject::wBasicObject * obj)
{
//	VecCard free_cards = _cardPhomBox[0]->getFreeCards();
//	for (Card* c : free_cards) 
//	{
//		if (c->GetBound().containsPoint(p) && NODE(c)->getTag() != 1)
//		{
...
void PhomScreen::ActionTouchEndObject(Utility::wObject::wBasicObject * obj)
{
    Card * c = (Card*)obj;
...
void PhomScreen::onDownPhom(std::vector<Phom> phoms)
{
	...
	if (_cur_move_player != _myInfo->GetIDDB()) {
		...
		SetInteractiveWidget(p_btn_down_phom,true);
		SetInteractiveWidget(p_btn_change_phom,true);
		SetInteractiveWidget(p_btn_create_phom,true);
...
void PhomScreen::actionDownPhom()
{
	if (checkSelectCardHaveAllEat()) {
		...
		SetInteractiveWidget(p_btn_down_phom,false);
		//cardPhomBoxs[0].setTouchable(Touchable.disabled);
		SetInteractiveWidget(p_btn_change_phom,false);
		SetInteractiveWidget(p_btn_create_phom,false);
...
void PhomScreen::onFinish(int finishPlayer, int freeCardScore, VecByte freeCards, int moneyPlayer) {
	...
	if (_countFinishPlayer == realPlayer) {
        ...
        if (_winnerID == _myInfo->GetIDDB()) {
//            doSomethingWhenIWin();
            PLAY_SOUND("SFX_GAMEBAI_THANGBAI");
        } else {
            PLAY_SOUND("SFX_VOTAY");
            //GameController.playSound(iWinSound.VOTAY);
        }
...
void PhomScreen::onSendCard(std::vector<int> userIDs, VecByte userAddCards, std::vector<std::vector<unsigned char>> userAddPhomCards) {
	...
	if (_cur_move_player == _myInfo->GetIDDB()) {
		...
		showGameButton(false, false, false, true, false, false);
		SetInteractiveWidget(p_btn_send_card,false);
		SetInteractiveWidget(p_btn_change_phom,true);
		SetInteractiveWidget(p_btn_create_phom,true);
		//cardPhomBoxs[0].setTouchable(Touchable.enabled);
		//
...
void PhomScreen::OnCardCanSend(VecByte cards) {
    ...
    static_cast<Button*>(p_btn_send_card->GetResource())->setBright(cards.size() > 0);
    static_cast<Button*>(p_btn_send_card->GetResource())->setTouchEnabled(cards.size() > 0);
    static_cast<Button*>(p_btn_fire_card->GetResource())->setBright(!(cards.size() > 0));
    static_cast<Button*>(p_btn_fire_card->GetResource())->setTouchEnabled(!(cards.size() > 0));
	if (cards.size() > 0) {
		SetInteractiveWidget(p_btn_send_card,true);
		for (size_t i = 0; i < cards.size(); i++) {
			{
...
void PhomScreen::onMoveError(std::string info) {
	...
	case ACTION_EAT:
		...
		break;
	case ACTION_FIRE:
		SetInteractiveWidget(p_btn_fire_card,true);
		break;
		...
		if (seat > -1) {
			...
			if (userID == _myInfo->GetIDDB()) {
				...
				if (userPhomCards.at(i).size() == 0) {
					...
					// vào
                    //addCardsToBox(seat, userEatCards.at(i), FLOOR_PHOM, userEatCards.at(i));
					...
					for (size_t j = 0; j < userPhomCards.at(i).size(); j++) {
                        ...
                        //addCardsToBox(seat, phom._cards, FLOOR_PHOM, userEatCards);
...
//			int cardid = userEatCards.at(i).at(j);
//			if (cardid > -1 && cardid < _myDealCards.size()) {
//				getCard(cardid)->GetResource()->setColor(Color3B(251, 255, 171));
//			}
//		}
//	}
...
void PhomScreen::onSkip(int currentPlayer, int downFirst)
{
	setDownFirstPlayer(downFirst);
	setMovePlayer(currentPlayer);
	if (currentPlayer == _myInfo->GetIDDB())
	{
		...
		p_btn_change_phom->SetDisable(false);
		p_btn_create_phom->SetDisable(false);
		//con thieu truong hop da an rui
		if (_is_eat)
		{
			showGameButton(!_isDownMyPhom, false, false, true, false, false);
		}
		else
		{
			if (!_is_get_card)
			{
				showGameButton(!_isDownMyPhom, true, checkCanEat(_cur_fire_cardID > -1 ? 3 : 4), false, false, false);
			}
			else
			{
				showGameButton(!_isDownMyPhom, true, false, false, false, false);
			}
		}
		if (_is_get_card)
		{
			p_btn_fire_card->SetDisable(false);
			p_btn_fire_card->Visible(true);
			p_btn_get_card->Visible(false);
			p_btn_eat_card->Visible(false);
		}
	}
}

void PhomScreen::onPlaying(int inverval, int currentPlayer, int downFirst,
	std::vector<int> userIDs,
	std::vector<VecByte> userFiredCards,
	std::vector<VecByte> userEatCards)
{
	_totalTimes = inverval;
	_cur_move_player = currentPlayer;
	_down_first_player = downFirst;
	resetReady();
	_isViewer = true;
	_isStartGame = true;
	_isGameEnd = false;
	//_cardPhomBox[0]->setIsGameEnd(isGameEnd);
	//showCardPhomBoxs(true);
	setDownFirstPlayer(downFirst);
	setMovePlayer(currentPlayer);
	showGameButton(false, false, false, false, false, false);
	...
	for (int i = 0; i < userIDs.size(); i++)
	{
		...
		if (userID > -1 && seat > -1)
		{
			fireCards = userFiredCards[i];
			eatCards = userEatCards[i];
			//them bai danh cua no vao
			//addCardsToBox(seat, fireCards, FLOOR_FIRED, eatCards);
            for (byte c : fireCards) {
                _cardPhomBox[seat]->addFireCard(c);
            }
			//them bai an cua no vao
			//addCardsToBox(seat, eatCards, FLOOR_PHOM, eatCards);
            for (byte c : eatCards) {
                _cardPhomBox[seat]->addEatCard(c);
                if(K_VEC_CONTAIN(eatCards, c))
                {
                    getCard(c)->GetResource()->setColor(Color3B(251, 255, 171));
                }
            }
		}
	}
...
void PhomScreen::addCardToBox(int seat, int cardID, int floor, bool isEatCard, bool isUpdateMyPhoms)
{
    if (seat > -1) {
        ...
        if (c != NULL) {
            ...
            if (seat == 0) {
                ...
                bool update = false;
                if (floor == FLOOR_FREE) {
                    K_VEC_REMOVE(_myFreeCards, cardID);
                    K_VEC_REMOVE(_myDealCards, cardID);
                    _myDealCards.push_back(cardID);
                    _myFreeCards.push_back(cardID);
                    update = true;
                } else if (floor == FLOOR_FIRED && !K_VEC_CONTAIN(_myFiredCards, cardID)) {
                    _myFiredCards.push_back(cardID);
                    update = true;
                } else if (floor == FLOOR_PHOM && !K_VEC_CONTAIN(_myEatCards, cardID)) {
                    _myEatCards.push_back(cardID);
                    update = true;
                }
                if (update) {
                    if (isUpdateMyPhoms) {
                ...
                _cardPhomBox[seat]->addCard(cardID, floor);
                _cardPhomBox[seat]->layoutBox();
                // cardPhomBoxs[seat].layoutBox(true);
                //GameController.playSound(iWinSound.CHIABAI);
                PLAY_SOUND("SFX_GAMEBAI_CHIABAI");
            }
            if (floor == FLOOR_PHOM && isEatCard) {
               c->GetResource()->setColor(Color3B(251, 255, 171));
            }
        }
    }
}

void PhomScreen::addCardsToBox(int seat, VecByte cards, int floor, VecByte eatCards)
{
	if (seat > -1 && cards.size() > 0)
	{
        for (int i = 0; i < cards.size(); i++)
        {
            int cardID = cards.at(i);
            Card* c = getCard(cardID);
            if(c != NULL)
            {
                c->SetVisible(true);
                _cardPhomBox[seat]->addCard(cardID, floor);
                if(floor == FLOOR_PHOM && K_VEC_CONTAIN(eatCards, cardID))
                {
                    c->GetResource()->setColor(Color3B(251, 255, 171));
                }
            }
...
void PhomScreen::removeCardFromBox(int seat, int cardID, int floor, bool isUpdateMyPhoms)
{
    if (seat > -1) {
        Card* c = getCard(cardID);
        if (c != NULL) {
            ...
            if (seat == 0) {
                // là mình
                bool update = false;
                if (floor == FLOOR_FREE || floor == FLOOR_PHOM) {
                    bool b = K_VEC_CONTAIN(_myDealCards, cardID);
                    bool b1 = K_VEC_CONTAIN(_myFreeCards, cardID);
                    K_VEC_REMOVE(_myDealCards, cardID);
                    K_VEC_REMOVE(_myFreeCards, cardID);
                    bool b2 = false;
                    if (_isDownMyPhom)
                        b2 = removeCardInPhom(cardID);
                    if (b || b1 || b2)
                        update = true;
                } else if (floor == FLOOR_FIRED) {
                    bool b3 = K_VEC_CONTAIN(_myFiredCards, cardID);
                    K_VEC_REMOVE(_myFiredCards, cardID);
                    if (b3)
                        update = true;
                }
                if (update) {
                ...
                _cardPhomBox[seat]->removeCard(cardID, floor);
                _cardPhomBox[seat]->layoutBox();
            }
        }
    }
...
bool PhomScreen::removeCardInPhom(int cardId)
{
    bool temp = false;
    for (int i = 0; i < _myPhoms.size(); i++) {
        if (K_VEC_CONTAIN(_myPhoms.at(i)._cards, cardId)) {
            temp = true;
            K_VEC_REMOVE(_myPhoms.at(i)._cards, cardId);
        }
    }
    
    for(auto it = _myPhoms.begin(); it != _myPhoms.end(); /*it++ */)
    {
        if(it->_cards.size() < 3)
        {
            K_VEC_INSERT(_myFreeCards, it->_cards);
            _myPhoms.erase(it);
        }
        else
            it++;
...
bool PhomScreen::isCardInPhom(int cardID)
{
    for (int i = 0; i < _myPhoms.size(); i++) {
        if(K_VEC_CONTAIN(_myPhoms.at(i)._cards, cardID))
            return true;
...
void PhomScreen::showArrowAt(Card* card)
{
    _img_arrow->stopAllActions();
    _img_arrow->setAnchorPoint(Vec2(.5f, .0f));
    CallFunc* func = CallFunc::create([this, card](){
        card->SetTouchEnable(true);
        NODE(card)->setTag(2);
        Vec2 pos = card->GetResource()->getParent()->convertToWorldSpace(card->GetPosition());
        _img_arrow->setPosition(Vec2(pos.x + card->GetSize().x / 2, pos.y + card->GetSize().y + 20));
        _img_arrow->setVisible(true);
        _img_arrow->runAction(RepeatForever::create(Sequence::create(MoveBy::create(.2f, Vec2(0, -15.0f)), MoveBy::create(.5f, Vec2(0, 15.0f)), nullptr)));
    });
    this->runAction(Sequence::create(DelayTime::create(.5f), func, nullptr));
}

void PhomScreen::hideArrow()
{
    _img_arrow->stopAllActions();
