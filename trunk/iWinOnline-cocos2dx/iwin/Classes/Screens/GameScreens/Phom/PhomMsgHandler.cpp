...
void PhomMsgHandler::onMessage(iwincore::Message* msg)
{
    ...
    case IwinProtocol::SKIP_TIENLEN:
    {
        ...
        if (GetPhomScreen()->GetBoardID() == boardID &&
            GetPhomScreen()->GetRoomID() == roomID)
        {
            GetPhomScreen()->onSkip(currentPlayer, downFirst);
        }
    }
        break;
    ...
    case 66:// ù ,ù khan
    {
		...
		case IwinProtocol::PLAYING:
		{
			...
			*/
			...
			{
				int interval = msg->readByte();
				int currentPlayer = msg->readInt(); //thang dang danh
				int downFirst = msg->readInt(); // thang ha bai dau tien
				std::vector<int> userIDs; //danh sach player
				//bai da danh
				std::vector<VecByte> userFiredCard;
				//bai an duoc
				...
				while (!msg->isEndRead())
				{
					...
					userFiredCard.push_back(cards);
					len = msg->readByte();
					VecByte cards1;
					for (int i = 0; i < len; i++)
					{
						cards1.push_back(msg->readByte());
					}
					userEatCards.push_back(cards1);
				}
				GetPhomScreen()->onPlaying(interval, currentPlayer, downFirst, userIDs, userFiredCard, userEatCards);
		...
		case IwinProtocol::QUICK_PLAY:
		{
			...
			{
				GetPhomScreen()->onSomeOneQuickPlay(userId, isQuickPlay);
			}
		}
		break;
		default:
