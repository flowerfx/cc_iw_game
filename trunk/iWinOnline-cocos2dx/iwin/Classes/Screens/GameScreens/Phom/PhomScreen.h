...
public:
    ...
    virtual void ActionTouchEndObject(Utility::wObject::wBasicObject * obj)										override;
	...
	/*
	* tra ve khi dang choi, co thang thoat ra, de xac dinh toi luot thang nao
	*/
	void onSkip(int currentPlayer, int downFirst);
	/*
	* vao ban dang choi 
	*/
	void onPlaying(int inverval, int currentPlayer, int downFirst,
...
private:
    ...
    void showArrowAt(Card* card);
    void hideArrow();
    
	/**
	* dung de add 1 luc nhieu la bai vao coc bai(chi duoc dung khi
	* playing, reconnect  va chi dung de them bai cua nguoi khac)
	*/
	void addCardsToBox(int seat, VecByte cards, int floor, VecByte eatCards);
    /**
     * được dùng để add bài vào nọc
     */
    void addCardToBox(int seat, int cardID, int floor, bool isEatCard,
                      bool isUpdateMyPhoms);
    
    /**
     * xóa bài khỏi nọc
     */
    void removeCardFromBox(int seat, int cardID, int floor, bool isUpdateMyPhoms);
    bool removeCardInPhom(int carID);
    /**
     * dùng để check coi có phải là bài trong phỏm ko
     */
    bool isCardInPhom(int cardId);
};
