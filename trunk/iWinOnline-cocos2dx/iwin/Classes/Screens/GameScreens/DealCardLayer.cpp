...
void DealCardLayer::dealCard(float delay, Card* c, std::function<void()> actionDone)
{
	...
	Vec2 desPos = c->GetResource()->convertToWorldSpaceAR(Vec2::ZERO);
	...
	float time_deal = 0.2f;

	NODE(free_card)->runAction(Sequence::create( DelayTime::create(delay),
		Spawn::create(MoveTo::create(time_deal, desPos), ScaleTo::create(time_deal, des_scale), RotateBy::create(time_deal, des_rotate) , nullptr) ,
...
void DealCardLayer::moveCard(Card* c, Vec2 des_pos, Vec2 des_size, const std::function<void()>& actionDone)
{
    ...
    CCLOG("111 === moveCard %d to destination %.2f", c->GetCardName(), des_pos.y);
	...
	NODE(free_card)->runAction(Sequence::create(Spawn::create(MoveTo::create(0.2f, target_pos), ScaleTo::create(0.2f, scalex, scaley), nullptr),
		CallFunc::create([c, free_card, actionDone]()
	{
		free_card->SetVisible(false);
        c->SetPosition(NODE(c)->getParent()->convertToNodeSpace(free_card->GetPosition() - free_card->GetSize()/2));
		actionDone();
	}), nullptr));
