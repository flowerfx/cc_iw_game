...
void XiZachPlayer::CreateCard(WidgetEntity* parent, Vec2 init_pos,int numCard)
{
	...
	p_parent = parent;
	bool isMe = (this->IDDB == GameController::myInfo->IDDB);
	for (int i = 0; i < numCard; i++)
	{
		...
		AnimUtils::RunEffectSetSizeDefaultBaiCao(c, isMe);
	...
	for (auto c : cards)
	{
		...
		if (c)
		{
			...
			AnimUtils::RunEffectChiaBaiCao(
				...
				[this]() {
				PLAY_SOUND("SFX_GAMEBAI_CHIACHAM");
			});
			idx++;
			c->SetVisible(true);
		}
...
void XiZachPlayer::FlipCard(std::vector<char> ids, std::function<void()> callBack)
{
	if (cards.size() < 0
		|| !cards.at(0)
		|| ids.size() <= 0
	...
	for (auto id : ids)
	{
		...
		if (id != cards[idx]->getID() && id != -1)
		{
		   //c->EventActionFinish();
			...
			{
				AnimUtils::RunEffectFlipBaiCao(cards[idx], id, [idx, list_pos_move_to, c, id, callBack]() {
					c->EventActionFinish();
					c->ResetID(id, DECK_TYPE::DECK_BAICAO);
					callBack();
				});
...
void XiZachPlayer::AddRetrieve(char id)
{
	...
	if (cards[numVisiable])
	{
		...
		{
			NODE(cards[numVisiable])->setZOrder(numVisiable);
		}
