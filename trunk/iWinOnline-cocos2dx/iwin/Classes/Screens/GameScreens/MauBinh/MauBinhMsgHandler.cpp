...
{
	if (GetMauBinhScreen()->isDelayLoadUi())
	{
		iwincore::Message* msgtmp =  new iwincore::Message(msg);
		if (msgtmp->getCommandId() == 40 && msgtmp->readByte() == 10)
		{
		...
		{
			GetMauBinhScreen()->pushDelayMessageUI(msg);
		}
		delete msgtmp;
		return;
	}
	...
	switch (msg->getCommandId())
	{
		case 47: // command game binh
		{
			...
			CCLOG("Mau Binh subCommand = %d", (int)subCommand);
			switch (subCommand)
			{
				case 20:  // Start game
				{
                    ...
                    //fake card to test bug logic 5 pair --> lung
//                    	myCards[0] = 4;
//                    	myCards[1] = 5;
//                    	myCards[2] = 12;
//                    	myCards[3] = 14;
...
//                    	myCards[12] = 51;
				...
				case 15: {
                    ...
                    GetMauBinhScreen()->setBoardInfo(limitTime, restTime);
				}
				break;
				case 25:// Auto binh.
				{
...
//                    int moneyType = Constants.BASE_MONEY;
//                    try {
//                        moneyType = msg.readInt();
//                    } catch (Exception e) {
//                        Log.log("BinhMsgHandle", e.getMessage());
//                    }
//                    Gdx.app.log(" ", "userId =  " + userId + " money = " + money + " moneyType" + moneyType);
//                    getGameScreen().addMoneyPlayerToList(userId, money, moneyType);
                }
                    break;
				default:
		...
		break;
		case IwinProtocol::GET_TIME_BAR:// update start time
		{
			ubyte timeStart = msg->readByte();
			GetMauBinhScreen()->OnShowCountDown(timeStart, .0f);
			GetMauBinhScreen()->showTextWaiting(false);
		}
		break;
		...
		case IwinProtocol::QUICK_PLAY:
		{
			...
			{
				GetMauBinhScreen()->onSomeOneQuickPlay(userId, isQuickPlay);
			}
		}
		break;
	}
