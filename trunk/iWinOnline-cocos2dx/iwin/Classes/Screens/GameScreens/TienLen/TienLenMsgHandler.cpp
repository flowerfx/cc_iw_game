...
void TienLenMsgHandler::onMessage(iwincore::Message* msg)
{
	...
	switch (msg->getCommandId()) 
	{
		...
		case IwinProtocol::QUICK_PLAY:
		{
			...
			{
				TienLenScreen::getInstance()->onSomeOneQuickPlay(userId, isQuickPlay);
			}
			break;
		}
		break;
...
void TienLenMsgHandler::forceFinishCommand(iwincore::Message* msg)
{
	...
	{
		int ower_id = msg->readInt();
		std::vector<char> cards;
		for (int i = 0; i < NUMBER_CARD_TL; i++)
		{
			cards.push_back(msg->readByte());
		}
		auto typeForceFinish = msg->readByte();
		TienLenScreen::getInstance()->ForceFinish(ower_id, cards, typeForceFinish);
	}
}
