...
void BaiCaoPlayer::DealCard()
{
	...
	float endX = p_parent->GetSize().x;
	float time = 0.1f;
	int idx = 0;
	std::vector<Vec2> list_pos_move_to = CardUtils::AligneCardPosition(startX, startY, endX, cards.size(), cards[0]->GetSize());
	for (auto c : cards)
	{
		if (c) 
		{
...
void BaiCaoPlayer::FlipCard(std::vector<char> ids)
{
	...
	for (auto id : ids)
	{
		...
		{
			AnimUtils::RunEffectFlipBaiCao(cards[idx], id,[c,id , idx]() {
				c->EventActionFinish();
