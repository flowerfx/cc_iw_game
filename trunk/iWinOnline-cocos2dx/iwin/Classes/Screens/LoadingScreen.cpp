...
{
	p_panel_loading_process = GetWidgetChildByName("loading_screen.loading_screen_1.loading_process");
	p_layout_img_download = GetWidgetChildByName("loading_screen.title_img_layout");
	auto text_version = GetWidgetChildByName("loading_screen.loading_screen_1.loading_txt");
	static_cast<TextWidget*>(text_version)->SetText(L"Phiên bản " + APPINFO->getVersion(), true);
	return 1;
}

void LoadingScreen::OnFadeInComplete()
{
...
int LoadingScreen::Update(float dt)
{
	...
	{  
		...
		TextureMgr->LoadTextureAtlas("atlas_card");
...
void LoadingScreen::OnProcessWidgetChild(const RKString & name, UI_TYPE type_widget, WidgetEntity * _widget)
{
}
