...
void IwinScrollView::InitNewWidget(int idx)
{
	auto w_entity = _item->Clone();
	if (_func_init_item)
	{
		_func_init_item(this, w_entity, idx);
	}

	_list_items.push_back(w_entity);
	_container->addChild(w_entity->GetResource());
}
