...
#include "Screens/GameScreens/Xam/XamMsgHandler.h"
...
int				GameController::typeMoneyNohu = BASE_MONEY;

int				GameController::currentScreenId = -1;
int				GameController::lastScreenId = -1;

GameScreen* GameController::currentGameScreen = nullptr;
std::vector<Player*> GameController::listPlayerRubyInGame;

bool GameController::isMyUser(int id)
{
	return myInfo->IDDB == id;
}
...
std::string GameController::getCurrentGameName()
{
	...
	{
	case GAMEID_TIENLEN:
		return LangMgr->GetString("killer_13s").GetString();
	case GAMEID_POKER:
		return LangMgr->GetString("poker").GetString();
	case GAMEID_CARO:
		return LangMgr->GetString("caro").GetString();
	case GAMEID_TETRIS:
		return LangMgr->GetString("tetris").GetString();
	case GAMEID_COVUA:
		return LangMgr->GetString("chess").GetString();
	case GAMEID_BAICAO:
		return LangMgr->GetString("3_cards").GetString();
	case GAMEID_BAICAORUA:
		return LangMgr->GetString("cao_rua").GetString();
	case GAMEID_TIENLEN_MB:
		return LangMgr->GetString("killer_13n").GetString();
	case GAMEID_TIENLEN_SOLO:
		return LangMgr->GetString("13s_solo").GetString();
	case GAMEID_BINH:
		return LangMgr->GetString("chinese_poker").GetString();
	case GAMEID_BLACKJACK:
		return LangMgr->GetString("blackjack").GetString();
	case GAMEID_PHOM_MB:
		return LangMgr->GetString("rummyvn").GetString();
	case GAMEID_CHESS:
		return LANG_STR("chinese_chess").GetString();
	case GAMEID_CO_UP:
		return LANG_STR("hidden_chess").GetString();
	case GAMEID_BAUCUA:
		return LANG_STR("bau_cua").GetString();
	case GAMEID_POKER_HK:
		return LANG_STR("poker_hk").GetString();
	case GAMEID_LIENG:
		return LANG_STR("lieng").GetString();
	case GAMEID_CATTE:
		return LANG_STR("catte").GetString();
	case GAMEID_WORLDCUP_COPHIEU:
		return LANG_STR("WC cổ phiếu").GetString();
	case GAMEID_WORLDCUP_CUOCCHAUA:
		return LANG_STR("bet").GetString();
	case GAMEID_DOMINO:
		return LANG_STR("Domino").GetString();
	case GAMEID_VONGQUAYTAILOC:
		return LANG_STR("gamename_luckycircle").GetString();
	case GAMEID_GIAI_DAU_CO_UP:
		return LANG_STR("tournament").GetString();
	case 32:
		return LANG_STR("Tù Xì").GetString();
	default:
...
void GameController::showGameScreen(s16 roomID, s16 boardID, int ownerID, int money, std::vector<Player*> players)
{
	...
	if (GameController::currentGameScreen != nullptr )
	{
		...
		{
			//handle reconnect with otp require
			ScrMgr->onHideOTPPageIfVisible();
			//
...
void GameController::createGameServiceHandlerOf(int gameId)
{
	...
	case GAMEID_CATTE:
		...
		break;
	case GAMEID_XAM:
		GameLogicHandler::gameHandlers =XamMsgHandler::getInstance();
		break;
	case GAMEID_TIENLEN:
	case GAMEID_TIENLEN_MB:
	case GAMEID_TIENLEN_SOLO:
...
void GameController::showReplay( int idx, int current_game_id)
{
	setGameType(current_game_id);
	GameLogicHandler::isJoinGame = true;
	GameController::currentGameScreen = static_cast<GameScreen*>(ScrMgr->GetGameScreen());
	if (ScrMgr->GetCurrentMenuUI(0) != HUD_SCREEN)
	{
		GameController::currentGameScreen->updateTextWithMoneyType();
		createGameServiceHandlerOf(gameId);
		ScrMgr->SwithToGameScreen(getCurrentGameType(), 2);
		static_cast<CoTuongScreen*>(GameController::currentGameScreen)->setupCoPho(idx);
	}
}

int GameController::GetFriendAvatar(int IDDB)
{
	if (IDDB > -1)
	{
