...
void SpriteIapMgr::registerDownload(SpriteIap* spr, std::string url, std::string name_save)
{
	...
	else
	{
		...
		SocialMgr->DownloadFile(info->url.c_str(), info->name_save.c_str(), CC_CALLBACK_3(SpriteIapMgr::doneDownload, this) , false);
	}
}
...
bool SpriteIapMgr::isDownloading()
{
	for (auto it = _map_download.begin(); it != _map_download.end(); it++)
	{
		if (it->second->download_state == SPR_DOWNLOAD_STATE_DOWNLOADING)
		{
			return false;
		}
	}
