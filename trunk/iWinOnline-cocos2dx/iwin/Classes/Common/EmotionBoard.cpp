...
void EmotionBoard::initWithSize(cocos2d::Size size)
{
	...
	_layout_container->setBackGroundImage("loadingscreen/bkg_thongbao.png", Widget::TextureResType::PLIST);
	...
	Vec2 delta_scale = Vec2(Director::getInstance()->getWinSize().width / 960.f, Director::getInstance()->getWinSize().height / 540.f);
	emo_size = cocos2d::Size(emo_size.width * delta_scale.x, emo_size.height * delta_scale.x);
	int num_col = size.width / ( emo_size.width + padding*delta_scale.x * 2 );
	...
	for (int i = 0; i < NUM_EMOTION; i++)
	{
		...
		btn_emo->setScale(delta_scale.x);
