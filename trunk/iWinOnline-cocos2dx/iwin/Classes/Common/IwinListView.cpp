...
void IwinListView::reload(int num_item)
{
	if (_list_items.size() < (size_t)num_item)
	{
		...
		for (int i = 0; i < num_more; i++)
		{
			...
			//chien todo: fix the item is list release cause crash
			if (_list_items[i]->GetResource()->getReferenceCount() <= 2)
			{
				_list_items[i]->GetResource()->retain();
			}
...
bool IwinListView::isVisible()
{
	return _list_view->Visible();
}

Utility::UI_Widget::ListViewWidget* IwinListView::getListView()
{
	return _list_view;
}