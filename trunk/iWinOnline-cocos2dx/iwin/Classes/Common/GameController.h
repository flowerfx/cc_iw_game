...
public:
	...
	static void showReplay(int idx, int current_game_id);
	...
	* type money tai xiu
	*/
	static int typeMoneyNohu;

	//screen id
	static int currentScreenId;
	static int lastScreenId;

	static int GetFriendAvatar(int IDDB);
	static std::vector<Player*> listPlayerRubyInGame;
	static bool isMyUser(int id);

};
#define  isMe(id) GameController::isMyUser(id)
