...
namespace AnimUtils
{
	void RunEffectFlopCardReturn(Card * c, Vec2 point, cocos2d::CallFunc* call_back)
	{
		...
		NODE(c)->runAction(Sequence::create(

			//MoveTo::create(0.2f, point),
			CallFunc::create([c , point]() {c->SetPosition(point); }),
			call_back,nullptr));
	}

	void RunEffectChiaBaiCao(Card* c, Vec2 point, float delay_time, bool isMyInfo, const std::function<void(void)>& call_back)
	{
		xml::BasicDec dec;
		...
		if (isMyInfo)
		{
			...
			c->SetActionCommandWithName("CHIA_BAI_CAO", CallFunc::create([c , call_back]() {
				c->EventActionFinish();
				if (call_back)
				{
					call_back();
		...
		else
		{
			...
			c->SetActionCommandWithName("CHIA_BAI_CAO_1", CallFunc::create([c, call_back]() {
				c->EventActionFinish();
				if (call_back)
				{
					call_back();
	...
	void RunEffectSetSizeDefaultBaiCao(Card * c, bool isMe)
	{
		//
		float scale_ratio = GetGameSize().width / 960.f;
		if (isMe)
		{
			//c->SetActionCommandWithName("DEFAULT_SIZE_BAI_CAO");
			c->SetSize(Vec2(97.5 * scale_ratio, 130 * scale_ratio));
		}
		else
		{
			//c->SetActionCommandWithName("DEFAULT_SIZE_BAI_CAO_1");
			c->SetSize(Vec2(60 * scale_ratio, 80 * scale_ratio));
		}
	}

	void RunEffectFlipBaiCao(Card* c, int id, const std::function<void(void)>& call_back)
	{
		CCLOG("BAI CAO Id flip: %d", id);
		//c->EventActionFinish();
		c->SetActionCommandWithName("FLIP_BAI_CAO", CallFunc::create([call_back , c]() {
			c->EventActionFinish();
			if (call_back)
			{
				call_back();
	...
	void RunEffectChiaBai(Card* c, Vec2 point, float delay_time, const std::function<void(void)>& call_back)
	{
		xml::BasicDec dec;
		dec.InsertDatafloat("DELAY", delay_time);
		dec.InsertDataVector2("POSITION", point);
		...
		c->SetActionCommandWithName("CHIA_BAI", CallFunc::create([call_back, c] (){
			c->EventActionFinish();
			if (call_back)
			{
				call_back();
	...
	void RunEffectMoveToFire(Card* c, Vec2 from_pos, Vec2 to_pos, const std::function<void(void)>& call_back)
	{
		xml::BasicDec dec;
		...
		c->SetActionCommandWithName("MOVE_CENTER_FRONT", CallFunc::create([c , call_back]() {
			c->EventActionFinish();
			if (call_back)
			{
				call_back();
